...
## Role integrity — the who's-who check (added 2026-08-10, MK order)

Motivated by the mixed-dyad pilot ([[mixed-dyad-pilot-2026-08-10]]): 2/4 giraffe×jackal dialogues **swapped the registered roles** — the giraffe side claimed the partner's grievance in first person; the jackal side confessed to the act it suffered — and `t_hear`, which credits card-need words reflected by the other party, has no defence against crediting a swapped conversation.

- **Mechanism:** per-bundle **role anchors** — (owner, pattern, label) triples of first-person claims the registered event assigns to exactly one party (B2 e.g. `"I made the changes"` → Mara; `"without my input"`, `"I saw the changes"` → Theo). A turn violates when a speaker other than the owner matches. Fixture-tested against the pilot's actual swap sentences.
- **Discipline:** read-only stored fields (`role_claim_violations`, `role_claim_violation`) in the degeneration-detector mold — never aborting, thresholded at analysis time; the anchor lexicon is content-hashed into the run manifest (`role_anchor_lexicon_sha256`). Instrument config, **not stimulus**: nothing reaches any prompt or stimulus digest.
- **Known misses, accepted in v1:** second-person hand-offs ("how did you feel when you saw the changes?" — the pilot's propagation vector), quoted/negated uses. Conservative anchors: a non-empty result is strong evidence; an empty one is weak.

## Hearing provenance — copied vs guessed credit (added 2026-08-10, MK order "fix the judge")

The transcription loophole, from both 08-10 diagnostics: in giraffe–giraffe both parties self-label, so a reflection can restate the partner's own announced needs and earn `t_hear` with zero empathic inference. The fix is judge-side scoring, not a stimulus or pairing change:

- **Per hear event:** for every judge-matched card-need term, was the term previously stated (word-bounded, case-insensitive, exact term) by the heard party themselves? → `basis`: **inferred** (≥1 matched term never self-stated — the reflector guessed), **restated** (all were — transcription could explain the credit), or **unscorable** (no matched terms recorded).
- **Per dialogue:** `t_hear_basis` per chair, plus the strict endpoint **`dyadic_t_hear_inferred_only`** — dyadic hearing where every crediting event has basis `inferred` and none sits on a role-violating turn. **Both rates are always reported side by side; which is confirmatory is a pre-registration decision, not a summary default.**
- **Scope note:** self-statement matching is exact-term ("supported" in the heard party's own turn does not count as a self-statement of "support"), while the judge's need-match is approximate — so the basis **over-counts inference and loosens the strict endpoint**. Stated so the prereg weighs it; morphological-variant matching of self-statements is the v2 tightening if the loophole matters in practice.
