...
> **[CORRECTED same day — see [[fleshout-batch-2026-08-10]] S2.]** The "invisible in same-frame corpora" clause was wrong at larger n: the flesh-out batch's same-frame **G-G dialogues swapped too** (both seeds — "I noticed the changes too and felt a bit taken by surprise" from the submitter; mutual "why did *you* make these changes"). The swap is **echo-borne, not mixed-frame-borne**; mixed frames only made it vivid first. The actor-seat elicitation gap (giraffe with no grievance of its own borrows the partner's) stands.
...
**MK correction (same day): the sole inferred pair is not made of NVC feelings.** "Ignored" and "unvalued" are evaluations in feeling costume — other-action charges — and MK's own F-purity red-line (2026-07-13, [[grammar-spec]]) already encodes the ruling: `ignored` sits on the registered faux blacklist, and so does `taken for granted`, the phrase D4's echo chain carried from the jackal turn into a giraffe turn. `unvalued` is a **lexicon gap**: absent from the CNVC whitelist (so never a valid FEEL) but also absent from the faux blacklist — MK's pen whether to add it (`isolated`, which D3 t3 re-attributed to self, is a similar borderline in neither list). Net effect on M3: the provenance count stands (the words were not copied), but the quality downgrades — **the model's one act of inference happened in the faux-feeling register**, and F-purity can already measure this without any new build.
