...
- `three-instrument-2026-09-06` — `v_norm` and `bos_mass` are effectively one curve here (r **0.959** anli / 0.925 halueval) and share the halueval peak at block 26. On anli `v_norm` peaks at block 45 with `js`, not at `bos_mass`'s block 61 — the one cell where the third instrument sides with disagreement rather than sink mass.
