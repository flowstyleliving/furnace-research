...
- `three-instrument-2026-09-06` — **the tightest bos/v-norm coupling in the panel.** With the third instrument finally captured per layer, `v_norm` peaks at block 70 (0.914) on anli and block 26 (0.914) on halueval — the same blocks as `bos_mass`, tracing nearly the same curve (r **0.971** anli / 0.944 halueval). `js_no_bos` remains the odd one out. The mid-stack blind spot is a `js` blind spot specifically, not a panel-wide one.
