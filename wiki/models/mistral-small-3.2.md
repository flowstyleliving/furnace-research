...
- `depth-coverage-2026-08-31` — **a clean held-out win on the one evaluable task.** halueval returns **+0.0650** [0.020, 0.114] for the depth-targeted single instrument over the best fixed rung, with fully stable selection (`bos`@33 in 5/5 folds). anli is absent from this analysis for the same reason it failed the registered grid-B cell — the behavioural gate (the model commits `'To'`).
- `three-instrument-2026-09-06` — halueval only (anli is absent for the same behavioural-gate reason as the registered grid-B cell). `v_norm` peaks at block 29 (0.895), close to `bos_mass` at 33, with r **0.929**; `js_no_bos` sits apart at block 37.
