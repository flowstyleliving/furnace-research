...
| File | Role |
|---|---|
| [[paper/cc-draft.tex]] | 📄 **THE merged CC paper (2026-07-26)** — single comprehensive Overleaf-ready LaTeX: sealed 18/20 core + the FULL six-task BENCH extension (families A/B/C as subsections of §``The registered BENCH extension''; A1 10/10 PASS, A2 6/10 FAIL sign-inversion, B1 7/20 FAIL stated prominently, B2 both-deployable, family-C descriptive) + post-seal scale/generation/locus + precision deconfound. Parent title kept, subtitle nods to the six-task registration. All 14 bibitems (13 parent + `halueval`); all frozen-language sentences stated exactly once; forward-work reports the executed sign-flip coincidence screen (NULL, p=0.50). Uses all 8 figures. E3 claim limited to measured 50/100/150 budgets; HaluEval label cost carried as the 2026-07-26 descriptive estimate (knee at 150, 10/10 flat to 500 — labeled post-hoc, not registered). **Compressed 21→15pp (2026-07-28)** for the 10–15pp target: cut the endpoints table + the A1/A2-definition table (both subsumed by the Table-1 ledger) and the rank-mirror + scale-extension figures (redundant with tab:a2 / tab:ext); paired the four Results figures into two `subcaption` rows; de-duplicated the Rigor/Discussion/Limits prose; `\footnotesize` bibliography. Frozen language fully intact per a 4-lens adversarial workflow (CLEAN, zero claims dropped); post-audit fixes added the registered bars to the abstract/intro, normalized the sealed gemma-3-4b orphan to 0.403 everywhere, and cross-referenced two orphaned floats. ~~Type-3 figure fonts remain a flagged follow-up (needs `pdf.fonttype=42` regen).~~ **✅ CLOSED 2026-09-11** — `cc-figures/make_figures.py` now sets `pdf.fonttype=42` and suppresses `CreationDate`; all 8 figures re-rendered with **0 Type-3 objects** and two consecutive renders are **byte-identical**. Faithfulness verified by rebuilding under the *old* font setting and diffing against the shipped PDFs: the only difference was the timestamp. **Release revision 2026-09-11 (15→20pp, 0 undefined):** new **Appendix A** (all 29 signal definitions, capture positions, source files) makes the paper self-contained, so the three "in preparation" companion citations are no longer load-bearing; new **operating-points subsection** + abstract sentence; abstract/intro reordered to lead with the supplied-candidate construct; and three code-level corrections — RPV is a **late-window average**, `p_max` is **post-answer** confidence, and the false "applies to closed-weight models" claim removed. **Retitled 2026-09-11** — *Universal Floor* → *Cohort-Level Floor* (MK's call, on Seek's objection; the body says the floor only *partially* extends and B1 replication fails at 7/20). The registered endpoint name is untouched. Also: `cc-extend-draft.tex` now carries a **do-not-cite banner** — source comment plus a visible framed box — naming its two inherited defects (RPV-as-readout-only, and the single-position framing). See [[results/cc-operating-points-2026-09-11]] and [[references/commit-locus]]. **Revision 2026-09-12 (20pp, 0 undefined):** discloses that the reimplemented gemma-4 and GPU extractors compute RPV on the **readout alone**, not the late-window average — affecting gemma-4-12b, both Llama-3.3-70B winners and the precision ladder (method bullet, glossary caption, §Scale, Appendix A); cohort range corrected **1.7–8B → 1.7–12B** at two sites. Two-pass structure confirmed on every extractor. Astra-audited; see [[claims]] §11. |
...
| `_bundles/cc-paper-2026-09-12.zip` | 📦 **Current Overleaf upload bundle (2026-09-12)** — `cc-draft.tex` + the 6 referenced figure PDFs, nothing else. **Verified by a clean-room compile** from the extracted zip in an empty directory: 20pp, 0 undefined references, decoded PDF title correct, edited text confirmed by `pdftotext` with a positive control. Also copied to `~/Desktop/furnace-overleaf/cc-paper.zip` (the 2026-09-08 bundle kept there as `cc-paper.zip.bak-0908`). ⚠️ `furnace-all-papers.zip` on the Desktop still holds the stale 2026-09-08 CC files. |
| `raw/papers/cc-paper-2026-09-12.pdf` | 📕 **Compiled PDF (20pp)** from that bundle; same file at `~/Desktop/furnace-overleaf/cc-paper.pdf`. |
| `cc-paper-2026-07-26.zip` | 🗄️ Superseded 2026-09-12. Was the Overleaf upload bundle — the ONE merged paper (tex + 6 referenced figures; fig5/fig7 cut in the 07-28 compression). Rebuilt 2026-07-28. Supersedes the two 07-23 bundles. Rebuilt 2026-07-26 after the Codex gpt-5.6-sol adversarial review (2 nits fixed: Discussion transfer-overclaim de-clawed; "≥150 labels" re-bounded to measured-budget phrasing) + date bump (`this version: July 26, 2026`). |
...
| [[paper/dc-scaffold]] | 🪺 Thesis, status-tagged headline claims, figure/table inventory, open decisions. Created 2026-08-17. **Carries two post-draft input sections, both unregistered and deliberately NOT in the manuscript.** *2026-08-29* ([[results/instrument-colocation-2026-08-29]]): the money figure's `bos_mass` 0.915 > js 0.897 > readout 0.816, and "placement is per-model *and per-instrument*" for the deployment corollary. *2026-08-31* ([[results/depth-coverage-2026-08-31]], [[results/instrument-redundancy-2026-08-30]]) **supersedes the 2026-08-29 section on what a limitations sentence should say**: the depth-coverage discriminator came back SPLIT (6/8 grid A, 6/9 grid B with a significant loss), so a claim-6 strengthening that ignores it would overstate the follow-on work; the cheapest robust sentence is instead **fixed-rung fusion loses to the best single fixed-rung column** (−0.0435 / −0.0383), which needs no depth argument. MK decision still owed; default recommendation unchanged: leave the 12pp draft frozen. |
