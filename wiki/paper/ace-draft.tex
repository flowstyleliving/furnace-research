...
\date{Original draft 2026-05-30 \quad This version: 2026-09-08}
...
per query. PRI v3 \citep{kitti2026pri} watched the
...
value-weighted formulations. Independent support for reading at this
\emph{position}, arriving from the output-distribution side rather than
the morphology side, comes from \citet{yeom2026commitment}: aligning
generations on the step at which an answer is emitted, they find
correctness-predictive information in the distribution localised to
that step, rising to $\auroc = 0.58$ there and decaying to chance on
either side, while token probability stays flat across the same window.
They do not read attention as a detector --- their only attention
measurement is a descriptive instruction-tuned-versus-base contrast at a
fixed step --- but they do propose attention concentration on the
question tokens as one of the candidates worth comparing for locating
that step, having found token entropy to be a poor localiser of it
($20\%$ exact match on their long-form trajectories). We note the
convergence on position and claim nothing from it about the channel.
...
Semantic entropy probes \citep{kossen2024sep} remove that test-time
cost by fitting a supervised linear probe on the residual stream to
predict binarised semantic entropy from a single generation; the
$10\times$ sampling cost is thereby relocated to calibration time
rather than eliminated, since each training query still requires
$\sim 10$ generations, and the probe is refit per model. ACE is
single-pass \emph{and} reads unsupervised geometric cells, spending
labels only on selecting the winning cell, its sign, and its
threshold. That last axis, rather than pass count, is the one the
field is acting on: \citet{li2026shars} --- the same group's most
recent hallucination work --- treats the detector as a black box in a
segment-wise rejection-sampling loop and instantiates it with a
sampling-based semantic-entropy variant rather than a probe, giving as
their reason that it ``does not require training a new probe model'',
and running at roughly $10$--$40\times$ the cost of greedy decoding ---
a figure they present as a favourable accuracy--efficiency trade-off
against other inference-time methods rather than as a cost to minimise.
Test-time cheapness is therefore not the settled objective of this
line; freedom from a fitted, per-model training step is the property
being selected for.
...
\citep{kitti2026pri} introduced PRI v3, a Fisher-pullback
...
for selection-bias-corrected confidence intervals. The output is a
...
artifacts documented in earlier Furnace Research runs. All
...
of earlier Furnace Research work: cell transfer fails even
...
PRI v3 \citep{kitti2026pri} measured Fisher-pullback rupture
...
\bibitem[Li et al.(2026)]{li2026shars}
L.~Li, G.~Channing, S.~M. Bhat, G.~D. Jones, and Y.~Gal.
\newblock Building Reliable Long-Form Generation via Hallucination
Rejection Sampling.
\newblock In \emph{Proceedings of the 43rd International Conference on
Machine Learning (ICML)}, PMLR 306, Seoul, 2026.
\newblock \url{https://arxiv.org/abs/2606.03628}.
...
\newblock \url{https://arxiv.org/abs/2605.22007}.
...
runs 20 control samples per model
