...
practical safety problem. Several signals now read a model's internal state at
...
\textbf{What is measured here, stated up front.} Every task in this study supplies the candidate
under judgement in the prompt: the HaluEval families pair a correct against a hallucinated
answer, summary or response, ANLI supplies a gold entailment label, and TriviaQA is
paired against a distractor. The label being discriminated is therefore a property of the
\emph{supplied input}, not whether the model under test hallucinated in its own free generation.
Read every endpoint below as calibrated discrimination of that input property. Whether the same
geometry predicts a model's own errors is a different question this design does not test.
