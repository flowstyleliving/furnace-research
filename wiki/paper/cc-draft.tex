...
\hypersetup{
  pdftitle={No Universal Detector, but a Cohort-Level Floor: Calibrated Early-Response Geometry for Input-Label Discrimination},
  pdfauthor={Michael S. R. Kitti},
  pdfsubject={Calibrated commit-moment geometry for input-label discrimination across ten language models},
  pdfkeywords={commit-moment geometry, input-label discrimination, calibration, attention morphology, readout geometry, pre-registration},
...
\title{\textbf{No Universal Detector, but a Cohort-Level Floor}\\[3pt]
\large A Pre-Registered Study of Calibrated Early-Response Geometry for
Input-Label Discrimination\\
...
\date{June--July 2026 (first version: July 26, 2026; this version: September 12, 2026)}
...
\begin{abstract}
\noindent
Several signals read a language model's internal state around the commitment of its first
answer token, each validated alone. We ask whether \emph{combining} them yields one discriminator
that transfers across models, or whether every model needs its own calibration. \textbf{What is
measured, stated first:} every task here supplies the candidate under judgement in the prompt.
The HaluEval families pair a correct against a hallucinated answer, summary or response; ANLI
supplies a premise and hypothesis whose gold label is entailment or contradiction; TriviaQA pairs a
question with either its answer or a distractor. The label discriminated is therefore a property of
the \emph{supplied input}, not whether the model under test hallucinated in its own free
generation. Read every endpoint below as calibrated discrimination of that input property; whether
the same geometry predicts a model's own errors is a question this design does not test.
...
pre-registrations spanning eight task registrations, ten open-weight models, and three labeled
constructs, the confidence-free geometric dispatcher clears its registered discrimination criterion
(\emph{deployable}: out-of-bag AUROC $95\%$ lower bound above $0.50$) on $\mathbf{18/20}$ sealed
...
Clearing that criterion is not an operating point: at a $10\%$ false-alarm rate the geometric
selector detects a median $0.25$--$0.70$ of positive items per task, and at a $10\%$ prevalence its
precision would be $0.21$--$0.42$ (descriptive, post-registration).
\end{abstract}
...
acknowledged uncertainty \cite{kalai2025hallucinate}. One promising approach reads a
signal not from the model's output text but from its internal state around the commitment of
...
\cite{farquhar2024semantic,agrawal2024references,wastl2025selfconsistency}. If such a signal also tracked a
model's \emph{own} unsupported answers, it could flag them without resampling---at the cost of the
prompt's own forward pass plus one more that includes the chosen answer token
(Appendix~\ref{app:signals})---and before that answer is shown to a user. It needs no fine-tuning, because it reads a frozen network; it does need
the weights, because it reads internal states, which in practice restricts it to open-weight models.
This paper tests a narrower, prior question: every task here supplies the candidate under judgement
in the prompt, so the own-answer case is not tested.
...
geometry---the centered softmax-Fisher spectrum, averaged over the readout and the last quarter of
blocks (Readout Pseudo-Volume, RPV) \cite{rpv}. Each established that the commit
...
extension (BENCH)---across eight task registrations and three labeled constructs, plus
...
registrations and three labeled constructs, plus post-seal probes. Verdicts use the
...
\texttt{gemma-3-12b}, \texttt{Qwen2.5-14B} (BC); \texttt{gemma-4}, $32$--$70$B, precision ladder (non-BC) & scale / generation / precision & orphan recovered by scale; locus is checkpoint-dependent; signal precision-invariant & descriptive\\
\bottomrule
\end{tabular}
\end{table}
...
\paragraph{The panel.} For each example we read four families of signals at the commit moment---an
umbrella covering three distinct reading positions, set out exactly in
Appendix~\ref{app:signals}---yielding a $29$-entry \emph{panel}:
...
value-vector norm weighted by the last query. These are read at the \emph{last prompt position},
before any answer token is chosen. \emph{Motivating hypothesis:} attention routes differently when
the supplied input is unsupported or contradictory---more mass on the sink, less agreement across
heads. The direction is not assumed; the selector fits it per deployment. (The attention locus has independent external support
\cite{binkowski2026sinks,vazhentsev2025rauq}; here it enters as one family in a cross-locus panel,
not a stand-alone detector.)
\item \textbf{PRI} --- residual-stream motion \emph{across} the answer token, summarized by
\texttt{null\_ratio}: the fraction of the final-layer state change, from the last prompt position to
the answer token's own position, lying \emph{off} the top direction of the probability-weighted
readout map. It is therefore available only once the answer token has been chosen and fed back
(Appendix~\ref{app:signals}). \emph{Motivating hypothesis:} a well-supported commitment moves the
hidden state mostly along the axis that writes the answer, an unsupported one spends more of its
motion off-axis. The nearest
...
\texttt{null\_ratio} instead reads the \emph{off}-subspace fraction of that motion as a
single scalar, calibrated rather than trained.
\item \textbf{RPV} --- the spread/curvature geometry of the output distribution (effective rank,
spectral entropy, and a rank-1 ``pseudo-volume'' of the centered softmax-Fisher spectrum). Each is
\emph{averaged over the readout and the logit-lens distributions of the last quarter of blocks}, at
the answer token's position---not read from the readout alone, as the name might suggest
(Appendix~\ref{app:signals}). The one exception is the non-byte-comparable extension cells of
Section~\ref{sec:scale}, whose reimplemented extractors compute RPV on the readout alone.
\emph{Motivating hypothesis:} a well-supported readout concentrates on
a few output directions, while an unsupported one spreads probability over many, inflating its
effective volume.
\item \textbf{Confidence} --- the model's own output confidence: \texttt{surprise}, the negative log
probability of the answer token at the last prompt position (under greedy decoding, the answer's own
confidence), and \texttt{p\_max}, the maximum probability of the distribution at the answer token's
position---which is the model's confidence about what \emph{follows} the answer, not about the
answer itself. \emph{Motivating hypothesis:} the familiar baseline, low probability or high surprise
flags a shaky answer; included so the geometric families can be measured \emph{against} it, not
merely alongside it.
\end{itemize}
...
detector; a \emph{deployment} is one (model, task) pairing. Appendix~\ref{app:signals} gives the
exact definition, capture position and source file of every signal; the companion reports
\cite{pri,ace,rpv} give motivating derivations but are not needed to reproduce anything here.
Figure signal names are decoded in Table~\ref{tab:glossary}.
...
\emph{deployable} if the OOB AUROC $95\%$ CI lower bound exceeds $0.50$.
\textbf{We keep this registered name and definition throughout, but a reader
should hold it to what it says.} It is a discrimination criterion, not an
operating point: a cell whose interval clears chance qualifies, and clearing
chance establishes neither an acceptable false-alarm rate nor practical
utility. Wherever this paper says \emph{deployable}, read \emph{clears the
registered discrimination criterion}. The selector never sees
...
disjoint) and the \emph{four} adversarial review passes noted above, a per-deployment shuffled-label
control re-runs the whole selection on three label permutations (within-stem swaps on paired tasks)
and fails the deployment if two or more clear chance; module and model-snapshot hashes pin
provenance.
...
\cite{triviaqa} correct-vs-wrong-answer judgment --- $n{=}200$ each, with the contradiction / wrong-answer
class as the positive class (label~$1$).
...
signal is a (layer, statistic) pair. Attention signals are read at the last prompt position and
\texttt{surprise} at the distribution there; the remaining readout signals are read at the answer
token's own position, after it is fed back (Appendix~\ref{app:signals} gives each exactly). The RPV
statistics are late-window averages in the byte-comparable cells shown here; in the reimplemented
extension cells of Section~\ref{sec:scale} they are read from the readout alone.
\emph{Layer prefixes:} \texttt{final} = last block, \texttt{last\_minus\_1} = penultimate,
\texttt{mid} = middle block.}
\label{tab:glossary}
\footnotesize
\begin{tabular}{lll}
\toprule
Statistic & Family & What it reads, and where \\
\midrule
...
\texttt{fisher\_eff\_rank}         & RPV        & effective rank of the centered Fisher spectrum, late-window average \\
\texttt{spectral\_entropy}         & RPV        & normalized entropy of that spectrum, late-window average \\
\texttt{neg\_shadow\_logvol\_r1}   & RPV        & rank-1 ``pseudo-volume'' (spread off the top axis), late-window average \\
\texttt{null\_ratio}               & PRI        & off-axis fraction of the state change across the answer token \\
\texttt{fusion\_rank\_mean\_geom}  & Fusion     & rank-mean of one representative per family (ACE, PRI, RPV) \\
\texttt{surprise}, \texttt{p\_max} & Confidence & surprise of the answer token / max probability at its position \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{But a cohort-level above-chance floor.} A pre-registered leave-one-model-out probe asks a
...
\emph{entailment/answer-correctness} tasks; whether the floor reaches a different
construct---judging whether a candidate answer is \emph{faithful to supplied source knowledge}---was
...
tuned on. A cohort-level floor under per-model calibration is a scientific statement; a transferable
...
benchmarks, now reproduced on a third construct.
...
\subsection{Baselines: does the panel earn its keep?}
\label{sec:baselines}

Two objections deserve direct answers: that the panel may be reading surface
form of the supplied candidate, and that its select-one-signal machinery may
not beat simply using every signal. Both were tested on the banked matrices
after the seal, so both are \textbf{descriptive and post-registration}; neither
moves an endpoint.

\paragraph{A lexical baseline loses on all six tasks.} TF-IDF (1--2gram)
with logistic regression on the raw prompt text, grouped by \texttt{stem\_id}
so a question never straddles a split, out-of-fold: $0.5022$
(\texttt{triviaqa\_paired\_rep}), $0.4007$ (\texttt{anli\_r2}), $0.6044$
(\texttt{anli\_r1\_rep}), $0.6834$ (\texttt{halueval\_qa}), $0.6421$
(\texttt{halueval\_summarization}), $0.7328$ (\texttt{halueval\_dialogue}),
against panel medians of $0.9095$, $0.6531$, $0.8410$, $0.8751$, $0.7481$ and
$0.7564$. \textbf{The two most informative cells are the ones where the
lexical model sits at or below chance while the panel does not}: on
\texttt{triviaqa\_paired\_rep} and \texttt{anli\_r2} there is no surface
shortcut available, and the geometry still reaches $0.910$ and $0.653$. One
cell is close to a wash and we report it: \texttt{halueval\_dialogue}, where
the panel leads by only $0.024$.

\paragraph{But selecting one signal is not the best use of the panel.} A
plain logistic regression on all $27$ banked signals \emph{beats} the
selector: pooled median $0.9627$ against $0.8946$, a paired difference of
$+0.0548$ $[-0.0005, +0.1138]$, winning $97\%$ of resamples. Under a
criterion fixed before the numbers were produced --- the selector is
justified only where it is reliably better than every simpler comparator ---
\textbf{the selector is not justified on $3$ of $6$ tasks, shows no reliable
difference on the other $3$, and is better on none.} A separate
implementation reproduces the direction on $9$ of $9$ model--task cells at a
mean of $+0.069$. Two single-family restrictions also match the full panel
with no reliable difference, which points at redundancy across the four
families.

This does not disturb the headline. Per-deployment selection still beats
fixed-cell transfer decisively ($4/6$ and $5/6$ tasks against leave-one-model-out
and leave-one-task-out fixed cells), so \emph{no universal detector} stands.
What it revises is an implicit design choice: on these data, the right use of
the panel is to \emph{combine} its signals rather than to select one.
\textbf{The comparison is not like-for-like} --- the panel figure is a
procedure-level OOB median with nested selection, the comparators are grouped
cross-validation --- so it bounds a design question rather than settling a
matched contest.

\subsection{Operating points: what clearing the criterion buys}
\label{sec:oppoints}

The registered criterion certifies ranking, not behaviour at a threshold, so it leaves a deployer
without the number they need. We therefore replayed every deployment's registered bootstrap with the
same draws and, inside each resample, set an alarm threshold at the $(1-\alpha)$ quantile of the
\emph{in-bag} negative scores, then applied cell, sign and threshold unchanged to the out-of-bag
rows. The positive class is the contradiction / wrong-answer / hallucinated candidate (label~$1$).
All $73$ deployments reproduce their published out-of-bag median, CI lower bound, resample count and
in-bag winner tally exactly, so Table~\ref{tab:oppoints} reports operating points of the same
procedure the endpoints certify---and, like the headline intervals, of the \emph{procedure} rather
than of any named signal. Descriptive and post-registration: no endpoint depends on it.

\begin{table}[tbp]
\centering\small
\caption{Operating points of the geometric selector, replayed on the registered resamples. TPR is
the out-of-bag detection rate of label-$1$ items at a threshold fitted to in-bag negatives; each
figure is the median across a task's deployments, with the per-deployment span in brackets at the
$10\%$ target. Precision is re-expressed at a $10\%$ prevalence, because the tasks themselves are
balanced $50/50$. Descriptive, post-registration; not a registered endpoint.}
\label{tab:oppoints}
\begin{tabular}{@{}lccccc@{}}
\toprule
Task & Cells & TPR @ $5\%$ FPR & TPR @ $10\%$ FPR & realized FPR & Precision @ $10\%$ prev. \\
\midrule
\texttt{anli\_r1} (core)         & $10$ & $0.161$ & $0.297$ $[0.19,0.60]$ & $0.121$ & $0.222$ \\
\texttt{triviaqa\_paired} (core) & $10$ & $0.386$ & $0.665$ $[0.25,0.96]$ & $0.111$ & $0.381$ \\
\texttt{anli\_r1\_rep}           & $8$  & $0.499$ & $0.629$ $[0.24,0.87]$ & $0.102$ & $0.406$ \\
\texttt{triviaqa\_paired\_rep}   & $10$ & $0.492$ & $0.670$ $[0.15,0.99]$ & $0.101$ & $0.422$ \\
\texttt{halueval\_qa}            & $10$ & $0.606$ & $0.701$ $[0.29,0.78]$ & $0.102$ & $0.424$ \\
\texttt{halueval\_summarization} & $8$  & $0.331$ & $0.418$ $[0.29,0.76]$ & $0.101$ & $0.309$ \\
\texttt{halueval\_dialogue}      & $9$  & $0.247$ & $0.379$ $[0.10,0.71]$ & $0.104$ & $0.279$ \\
\texttt{anli\_r2}                & $8$  & $0.155$ & $0.254$ $[0.17,0.51]$ & $0.106$ & $0.211$ \\
\bottomrule
\end{tabular}
\end{table}

Thresholds transfer: against a $10\%$ target the realized out-of-bag false-alarm rate is
$0.10$--$0.12$ in every task's median cell. Detection is another matter. At that false-alarm rate the
geometric selector flags a median of $0.25$ (\texttt{anli\_r2}) to $0.70$ (\texttt{halueval\_qa}) of
positive items, spanning $0.10$--$0.99$ across individual deployments; the three weakest cells are
exactly the three whose intervals do not clear the criterion (the two sealed ANLI orphans and
\texttt{Qwen3-1.7B} on \texttt{halueval\_dialogue}), which is a consistency check rather than a new
result. And because every task here is balanced, precision on these data flatters what a deployment
would see: re-expressed at a $10\%$ prevalence the same operating points give a median precision of
$0.21$--$0.42$ per task, so most alarms would be false. This is the concrete content of the caveat
attached to \emph{deployable} in the Method section: clearing chance is a discrimination result, not
a usable alarm.
...
The practical reading is blunt: a commit-moment discriminator of supplied-candidate faithfulness
can be calibrated for any of these ten models, but only with a labeled calibration set from
\emph{that} model to fix its orientation.
...
models, $1.7$--$12$B, all 4-bit MLX---``cohort'' is the load-bearing word in every universality
statement.

\paragraph{Benchmark contamination is not ruled out.} ANLI, TriviaQA and
HaluEval are public datasets that plausibly appear in the pretraining corpora
of some or all of the ten checkpoints, none of whose training data we can
inspect. A model that has seen an item may recognise it rather than judge it,
and recognition would be visible in commit-moment geometry just as judgement
would. We ran no contamination analysis and make no claim either way; readers
should treat every discrimination result here as an upper bound on what the
same geometry would achieve on genuinely unseen items. This limitation is
shared with most published work on these benchmarks, which does not make it
smaller.
...
\paragraph{Bottom line.} On a third construct, the registered verdict is
...
attention signal---plausibly reimplementation drift in the attention statistics. There is also a
difference by construction: this extractor computes the three RPV statistics on the readout
distribution alone, not as the late-window average of Appendix~\ref{app:signals}, so the RPV
component of that fusion signal is not the statistic used in the byte-comparable cells. Both are
reasons not to over-read the per-signal identity of these two non-byte-comparable cells.
...
\emph{not} byte-comparable to the seal and are never pooled with it. Like the \texttt{gemma-4}
extractor, this one computes the three RPV statistics on the readout distribution alone, not as the
late-window average of Appendix~\ref{app:signals}. Two observations follow. First,
...
$0.70$, TriviaQA $0.79$), yet \emph{both} of its winners are readout-only RPV statistics
(\texttt{neg\_shadow\_logvol\_r1} on ANLI, \texttt{fisher\_eff\_rank} on TriviaQA) at the answer
token's position---not the ACE attention morphology that wins every byte-comparable cell. The locus
that carries the signal is thus checkpoint-dependent: attention-side for the Qwen checkpoints tested,
readout-side for Llama-3.3-70B. With one or two checkpoints per family, this is not a family rule.
...
tests whether the signal is rounding noise. Its rungs run on the same GPU extractor, so their RPV
columns are readout-only as well. Judged on fixed cells rather than the selection-noisy
...
ACE attention. The checkpoint-dependent locus is thus not a precision artifact.
...
hallucination; its counterfeits are dataset-synthesized; and its cohort spans $1.7$--$12$B 4-bit
...
\emph{Architecture-Dependent Fisher Pullback Geometry at the Commit Moment: A Sealed
Pre-Registered Test on Synthetic Logical Contradictions}. Furnace Research, 2026 (in preparation).
The implementation used in this paper is vendored at
\url{https://github.com/flowstyleliving/commit-confluence} (\texttt{vendor/t0\_core}).
\bibitem{ace} M.~S.~R.~Kitti.
\emph{Attention Commitment Estimation for Pre-Generation Belief Readout}. Furnace Research, 2026
(in preparation). The implementation used in this paper is vendored at the repository above.
\bibitem{rpv} M.~S.~R.~Kitti.
\emph{Readout Pseudo-Volume: A Confidence-Independent, $\Delta h$-Free Commitment Signal That
Converges with a Prior Detector}. Furnace Research, 2026 (in preparation). The implementation used
in this paper is vendored at the repository above.
...
\appendix
\section{Signal definitions, capture positions and source code}
\label{app:signals}

The panel holds $29$ signals: the $27$ columns banked in every deployment's score matrix, plus two
fusion signals built from those columns before selection. All of it is public; the extraction code is
vendored in the repository byte-identically from the sealed state (\texttt{vendor/t0\_core/}), and
paths below are relative to it.

\paragraph{Three positions, not one.} Index the prompt's tokens $0,\dots,T-1$ (position $0$ is the
begin-of-sequence token) and let $p_j$ be the model's output distribution at position $j$. Decoding
is greedy, so the answer token is $a=\arg\max_i p_{T-1}(i)$ and occupies position $T$. The paper's
shorthand ``commit moment'' covers three distinct reading positions:
\begin{itemize}\setlength\itemsep{2pt}
\item \textbf{Attention signals} are read at position $T-1$, during the prompt's own forward pass,
\emph{before} $a$ is chosen.
\item \textbf{\texttt{surprise}} is $-\log p_{T-1}(a)$, also at position $T-1$; under greedy decoding
it equals $-\log\max_i p_{T-1}(i)$, so it is the answer's own confidence.
\item \textbf{Every other readout signal}---\texttt{null\_ratio}, the three RPV statistics and
\texttt{p\_max}---is read at position $T$, \emph{after} $a$ has been appended and the model has run
forward again. Hence $\texttt{p\_max}=\max_i p_T(i)$ is confidence about the token that would
\emph{follow} the answer, not about the answer.
\end{itemize}
The practical consequence is worth stating plainly: apart from the attention family and
\texttt{surprise}, nothing in this panel exists until the answer token has been selected. A detector
built on it is a one-token guard---it can withhold an answer already chosen---not a predictor of
what the model is about to choose.

\paragraph{ACE --- 21 signals.} Three blocks of an $N$-block model, zero-indexed
($\texttt{final}=N-1$, $\texttt{mid}=\lfloor N/2\rfloor$, $\texttt{last\_minus\_1}=N-2$), crossed
with seven statistics. Let $A^{(\ell,h)}$ be the attention distribution of query head $h$ at position
$T-1$ over positions $0,\dots,T-1$; let $H$ be the query heads, $H_{KV}$ the key--value heads,
$r=H/H_{KV}$, $\pi(h)$ the KV group of head $h$, and $\|V^{(\ell,k)}_i\|$ the $L_2$ norm of KV head
$k$'s value vector at position $i$. With
$\mathrm{JS}(P\|Q)=\tfrac12\mathrm{KL}(P\|M)+\tfrac12\mathrm{KL}(Q\|M)$ and $M=\tfrac12(P+Q)$:
\begin{align*}
\texttt{js}^{(\ell)} &= \tfrac{1}{H}\textstyle\sum_h \mathrm{JS}\big(A^{(\ell,h)}\,\big\|\,\bar A^{(\ell)}\big),
\qquad \bar A^{(\ell)}=\tfrac1H\textstyle\sum_h A^{(\ell,h)};\\
\texttt{js\_kv\_groups}^{(\ell)} &= \text{the same, over the $H_{KV}$ group means }
\tilde A^{(\ell,k)}=\tfrac1r\textstyle\sum_{j=1}^{r}A^{(\ell,(k-1)r+j)};\\
\texttt{js\_no\_bos}^{(\ell)} &= \text{the same as \texttt{js}, after deleting position $0$ and renormalising each head};\\
\texttt{bos\_mass}^{(\ell)} &= \tfrac1H\textstyle\sum_h A^{(\ell,h)}_0;
\qquad \texttt{v\_norm\_bos}^{(\ell)} = \tfrac{1}{H_{KV}}\textstyle\sum_k \|V^{(\ell,k)}_0\|;\\
\texttt{v\_norm\_max}^{(\ell)} &= \tfrac{1}{H_{KV}}\textstyle\sum_k \max_i \|V^{(\ell,k)}_i\|;
\qquad \texttt{v\_norm\_lastq\_weighted}^{(\ell)} = \tfrac1H\textstyle\sum_h\textstyle\sum_i A^{(\ell,h)}_i\,\|V^{(\ell,\pi(h))}_i\|.
\end{align*}
Source: \texttt{pri\_calibrator.py} (\texttt{\_compute\_attention\_score}) and
\texttt{scripts/diagnose\_inter\_head\_disagreement.py}.

\paragraph{PRI --- 1 signal.} $\texttt{null\_ratio\_post\_rank1}=\|\Delta-vv^{\top}\Delta\|/\|\Delta\|$,
where $\Delta=n(h_T)-n(h_{T-1})$ is the change in the final block's hidden state across the answer
token and $n$ is the model's own final RMSNorm with its learned gain. $v$ is the top right-singular
vector of $A=\mathrm{diag}\big(\sqrt{p_T(S)}\big)W_S$, where $S$ holds the $\min(256,V)$
highest-probability tokens under $p_T$ and $W_S$ the matching unembedding rows, so
$A^{\top}A=W_S^{\top}\mathrm{diag}(p_T(S))W_S$ is the \emph{uncentered} softmax-Fisher metric pulled
back through the unembedding. The probabilities are not renormalised on $S$. Source:
\texttt{pri\_runtime.py} (\texttt{compute\_step}, \texttt{null\_ratio\_and\_energy}), called with
centered capture disabled.

\paragraph{RPV --- 3 signals, each a late-window average in the byte-comparable cells.} For a distribution $q$, let $S_q$ be its
$512$ highest-probability tokens and $\tilde q$ the restriction of $q$ to $S_q$, renormalised to sum
to one. Let
$F(q)=W_{S_q}^{\top}\big(\mathrm{diag}(\tilde q)-\tilde q\tilde q^{\top}\big)W_{S_q}$ be the
\emph{centered} softmax-Fisher pullback, with eigenvalues $\lambda_1\ge\dots\ge\lambda_d\ge0$
(zero-padded to $d$). Writing $m$ for the size of the active set
$\{i:\lambda_i>10^{-12}\lambda_1\}$, $\tilde\lambda$ for the active eigenvalues normalised to sum to
one, and $\mathcal{H}=-\sum_i\tilde\lambda_i\log\tilde\lambda_i$:
\[
\texttt{fisher\_eff\_rank}(q)=e^{\mathcal{H}},\qquad
\texttt{spectral\_entropy}(q)=\frac{\mathcal{H}}{\log m},\qquad
\texttt{neg\_shadow\_logvol\_r1}(q)=\frac{1}{2(d-1)}\sum_{i\ge2}\log\!\big(\lambda_i+10^{-12}\big).
\]
\textbf{The banked column is not the readout alone.} Each statistic is evaluated on the readout
distribution $p_T$ \emph{and} on the logit-lens distribution $\mathrm{softmax}\big(W_u\,n(h^{(b)}_T)\big)$
of every block $b$ in the last quarter of the stack ($b=N-\lceil N/4\rceil,\dots,N-1$), and the banked
value is the mean of those $\lceil N/4\rceil+1$ numbers; a sample whose aggregate is incomplete is
dropped rather than averaged over fewer sources. The readout-only values are banked alongside but are
not what the selector sees. Source:
\texttt{exploratory/shadow-ambiguity/comprehensive\_run.py} (\texttt{trace\_pair\_features},
\texttt{\_spectrum\_stats}, \texttt{fc\_full\_spectrum}).
\textbf{Exception: the non-byte-comparable extension cells of Section~\ref{sec:scale}}
(\texttt{gemma-4-12b}, \texttt{Llama-3.3-70B}, and the precision ladder). Their reimplemented
extractors call \texttt{\_support\_spectrum} on $p_T$ directly, so in those cells each RPV column, and
the RPV component of each fusion column, is the \emph{readout-only} statistic. The two forward passes
are the same as above. Source, relative to the repository root: \texttt{stage\_b/gemma4\_full\_extract.py},
\texttt{stage\_b/gemma4\_readout\_extract.py} and \texttt{modal/modal\_app.py} (\texttt{extract}, identical
to the implementation that produced the GPU cells).

\paragraph{Confidence --- 2 signals.} $\texttt{surprise}=-\log p_{T-1}(a)$ and
$\texttt{p\_max}=\max_i p_T(i)$, at the two different positions described above.

\paragraph{Fusion --- 2 signals.} Each component is oriented by a sign frozen in
\texttt{stage\_b/fusion\_signs.json}---fitted on earlier, disjoint sealed-era data, per (model, task)
where available and cohort-modal otherwise---then rank-transformed within the deployment to
$(\mathrm{rank}-\tfrac12)/n$ and averaged. \texttt{fusion\_rank\_mean\_geom} averages
\texttt{last\_minus\_1\_js\_no\_bos}, \texttt{null\_ratio\_post\_rank1} and
\texttt{fisher\_eff\_rank}; \texttt{fusion\_rank\_mean\_full} adds \texttt{surprise} and is therefore
excluded from the confidence-free endpoint. A sample is \texttt{NaN} in a fusion column if any
component is. \textbf{One transductive detail, disclosed:} the rank transform runs over all rows of
the deployment before any resampling, so a fusion column depends on the \emph{scores}---never the
labels---of rows that later fall out of bag. Source: \texttt{confluence\_calibrator.py}
(\texttt{append\_fusion\_columns}).

\paragraph{Which signals each endpoint sees.} The full panel is all $29$. The confidence-free
geometric endpoint drops \texttt{surprise}, \texttt{p\_max} and \texttt{fusion\_rank\_mean\_full},
leaving $26$. Source: \texttt{confluence\_calibrator.py} (\texttt{NON\_GEOMETRIC\_KEYS},
\texttt{calibrate\_merged}).

Every per-deployment score matrix, every profile and the analysis scripts cited here are published
with the repository, so each definition above can be checked, and each number recomputed, without
running a model.
