...
\title{Depth Curves: where hallucination separation lives in the stack,\\
and why three probes are not enough to find it}
\author[ ]{Furnace Research}
\date{August 24, 2026}
...
$10$ instruction-tuned models spanning four families and $28$--$128$ blocks, on
two hallucination-adjacent benchmarks, with a shuffled-label envelope and
pre-registered decision bars frozen before any curve was inspected. Discovery on
$8$ cells and prospective held-out-model confirmation on $12$ more give three
results. First, \textbf{no placement rule was established}: peak location is
...
\textbf{NOT TESTED}. Three cells aborted, and each abort is itself a finding: a
behavioral gate, an instrument-domain boundary where the sealed metric is
undefined under total attention-sink collapse, and one true miss. For deployment
the corollary is narrow and practical: one cheap per-model per-layer calibration
pass replaces any depth heuristic, because in this cohort no heuristic survived.
\end{abstract}
...
grid~B ($12$ cells, four families, held-out models) is \emph{prospective
confirmation on two fixed benchmarks}, with every threshold, guard, and
...
\texttt{torch}/Modal extraction at \texttt{nf4} (one cell dequantised from FP8),
and is \emph{not} byte-comparable with our sealed MLX panels; the two are never
pooled. Grid~A rates are cited as discovery only and never enter a grid~B count.
...
\paragraph{The envelope.} For each cell we redraw the labels $200$ times under a
fixed seed and record the $97.5$th percentile of the resulting per-block
sign-free AUROC. This envelope is the paper's honesty floor: because $A_\ell$ is
...
all twelve cells reach a terminal status. Nothing here is a re-score, and no
threshold moved after the freeze.
...
third clears it by less than $0.01$. The band was not merely under-sampled; it
...
\begin{figure}[t]
  ...
  envelope. The dashed line is that panel's in-sample winner ($0.8156$, a readout
  cell --- a different instrument, shown for scale, not as a matched baseline).
...
terminal give-back genuinely is not there. It is visible at zero in
...
\textbf{Neither frozen placement rule fired, and the panel that replaced them
shows structure without a transferable law.} Grid~A registered two candidate
...
resolution, \emph{no placement rule was established}. We phrase it that way
deliberately. Failing to establish a rule at $n=200$ per cell across ten models
...
three-rung panel appeared to show separation peaking at $N-2$. It does not. $N-2$
merely won among three sampled points --- the same sampling failure that
\cref{sec:blindspot} documents, in its other guise. On the full curves the
\texttt{Qwen2.5} peaks land between $N-6$ and $N-17$, at depth fractions $0.79$
to $0.93$: too wide for either rule.
...
Llama band were both borne out: the mid-stack band is present at
\texttt{Llama-3.1-70B} and absent at \texttt{Llama-3.1-8B}. The registration
...
\begin{figure}[p]
  ...
  shuffled-label envelope. Lower block, blue: grid~B (confirmation), deliberately
  unshaded --- no envelope was registered for grid-B cells. Three grid-B cells
...
\begin{table}[t]
  ...
  \input{dc-figures/t1_verdict_table.tex}
...
\begin{table}[t]
  \centering\footnotesize
  \input{dc-figures/t2_endpoint_ledger.tex}
...
narrow, and it is the practical point of the paper. If no absolute rule and no
relative rule survives ten models, then a probe layer imported from another model
--- or from another size in the same family --- is an untested assumption. The
...
\cref{fig:money} for that deployment. Independent systems already do this. The
recall-controlled abort cascade of \citet{ruan2026abort} fixes its probe layer
per model by a preliminary per-layer sweep, landing at $14/28$ for a $3$B Llama
and $20/28$ for a $7$B Qwen --- and those two placements echo our own family
clusters, small-Llama mid and Qwen late, on two models we also measure.

The cost of not measuring is documented next door. \citet{apollo2025probes} train
a deception probe in the \texttt{Llama-3.3-70B} residual stream at layer $22$ of
$80$; their layer sweep shows the usable deployment band is mid-stack and narrow,
with recall at a fixed low false-positive rate collapsing within a couple of
layers while AUROC stays respectable. That is the same failure mode we report
from the other side: an aggregate score can look healthy at a layer where the
deployable operating point has already gone. Both of these are
\emph{different-construct, different-object} results --- residual-stream
directions rather than attention morphology, strategic deception rather than
hallucination --- and we cite them as convergent evidence about depth, never as
replication.
...
\begin{itemize}[leftmargin=1.4em, itemsep=0.2em]
  ...
  \item \textbf{Unbalanced panel.} Two models per family, family confounded with
    tokenizer, architecture and size. No factorial claim; the version comparison
    establishes no base-checkpoint identity.
  \item \textbf{One precision-heterogeneous cell.} \texttt{Mistral-Medium-3.5} is
    FP8-origin, deterministically dequantised to BF16, and flagged wherever it
