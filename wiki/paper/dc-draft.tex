...
\textbf{Four recent internal-state detectors span the full range from measuring the probe layer
to assuming it, and the assumption is the more common practice.} We group them on that axis
because it is the axis this paper is about. All four study \emph{deception} rather than
hallucination, and three read the residual stream rather than attention morphology, so none is a
...
Across the group: where placement is measured it is reported as model-specific and the usable
band is narrow; where it is assumed, it is assumed to be a constant fraction of depth. We are
aware of no published sweep supporting that constant, and \cref{sec:placement} is the closest
thing to a direct test of it --- neither frozen rule fired.
