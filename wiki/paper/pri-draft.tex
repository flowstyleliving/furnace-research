...
% Float placement. Every float is [htbp]; these fractions let LaTeX fit more
% of them per page before deferring, which keeps each figure near the text
% that references it without spending pages on half-empty float pages.
...
\author{Michael S.R. Kitti \\ Furnace Research (independent) \\ \texttt{msrkittty@proton.me}}

\date{Original draft 2026-04-27 \quad This version: 2026-09-08}
...
\begin{abstract}
Lightweight geometric measurements of a language model's state can
distinguish properties of the prompt it was given, but what such a
measurement licenses depends on score orientation, subspace rank, and
where in the response it is captured. We test
...
contradictions. \textbf{This benchmark provides too little valid outcome
variation to support an error-prediction evaluation: five of six models
answer at least 99.2\% correctly --- two of them make 2 and 5 errors in
600 --- and the sixth's apparent 19.3\% is a truncated-chain-of-thought
formatting artifact. Every result here therefore discriminates contradictory
\emph{prompts} that the models go on to answer correctly, and does not
demonstrate hallucination detection.} The head-to-head, the analysis plane (final
transformer layer, the first generated token, top singular direction),
the residualization, the bootstrap, and the pass bar were locked
before the reported evaluation. The rank was not in the original seal:
it was fixed at 1 by amendment after an exploratory sweep in which the
gate inverted from 0-of-3 to 3-of-3 primaries, and before the
replicate data existed. Across six 4-bit-quantized open-weight
...
non-overlap CI, and neither vendor nor parameter count sorts the six
models --- a description of the observed heterogeneity, not evidence
that they cannot predict it. The
...
\citep{hu2025harp}. Two earlier iterations of our own index --- v1, a
cosine rupture scaled by token surprise, and v2, a Fisher-distance
refinement --- together with a companion result localizing
contradiction-induced rupture to the first generated token, are
summarized below and superseded by this paper; v3 (null-space ratio)
is introduced here and tested head-to-head against a HARP-style
static-SVD baseline.

\paragraph{Prior PRI work, in brief.} This section is the canonical
record of the two earlier iterations; they circulated only as internal
Furnace Research drafts and are superseded here rather than cited.
PRI v1 defined a cosine-distance signal between consecutive hidden
states scaled by token surprise. It showed that the angular component
of the hidden-state shift between the last prompt position and the
...
with surface confidence. PRI v2 replaced the
cosine with a Fisher-information distance scalar, detaching rupture
from surprise and outscoring v1 on three primary models (Llama 3.2 3B,
Mistral 7B, Qwen 2.5 7B). That comparison was an internal preliminary
observation, not a pre-registered endpoint, and is reported here as
lineage rather than as a result of this paper. A companion result
localized the contradiction-induced
...
(Phi-3.5), an apparent within-model rank flip that we show is an
within-model transition with no supported mechanism (Gemma 4B), and within-model
...
PRI v1 (cosine $\dh$ scaled by token surprise)
and v2 (Fisher distance scalar) are
precursors of v3's null-ratio formulation, specified in
\Cref{sec:intro}; we report all four as
...
sampling-based. The pass-count contrast should not be overstated:
semantic entropy probes \citep{kossen2024sep} recover the
semantic-entropy signal from a single generation by fitting a
supervised linear probe on the residual stream, so the $10\times$
sampling cost moves to calibration time --- each training query still
needs $\approx 10$ generations --- rather than disappearing, and the
probe is refit per model. What separates PRI from that line is not the
number of passes but what is read: an unsupervised geometric property
of the commit step. The score itself has no fitted direction and no
supervision target; its evaluation \emph{orientation} is fitted on
labels, as \Cref{sec:methods:prereg} sets out, so a deployment would
need a sign calibrated out-of-sample.

\paragraph{Commitment as the locus.} Independent work supports reading
at the commit moment specifically. \citet{yeom2026commitment} align
generation trajectories on the step at which an answer concept is
emitted and report that the correctness-predictive information in the
output distribution is localised there --- at chance two or more steps
earlier, rising to $\auroc = 0.58$ at the emission step, and decaying
to chance within a few steps after --- while plain generated-token
probability stays flat across the same window. They also find that
among hallucinations carrying substantial probability mass on the
correct answer concept, what separates them from correct generations is
whether that mass is concentrated on a single surface form rather than
dispersed across alternatives. Their statistic requires ground-truth
answer aliases and is presented as an analytical probe rather than a
deployable detector, so it is not a comparator for anything measured
here; we cite it as convergent evidence for the choice of locus, from a
different construct and a different instrument.
...
model until it has produced its first generated token and read the
residual stream (the running-sum hidden vector that produces the
model's next-token distribution) at two adjacent positions: the last
prompt position and the first generation position. \textbf{A note on
timing, since the framing invites a stronger reading than the
measurement supports.} The first token must be sampled and fed back
before the second of those states exists, so the score is not
available \emph{before} the model selects that token --- it is
available before the token is \emph{released}. A deployed monitor
would buffer one token and decide whether to emit it. That is a
one-token-latency guard, not a pre-selection prediction, and every
claim in this paper should be read on those terms. We then score the
pair two ways: \textbf{Fisher pullback} --- a curvature-aware
...
curve}, a $[0, 1]$ score for binary classification, where $0.5$ is
chance, $1$ is perfect discrimination and $0$ is perfect
\emph{anti}-discrimination). \emph{Pre-registration}
means we locked the analysis plane, the
...
fit to the result. One parameter escaped that lock: the rank was fixed
at $1$ later, by a disclosed amendment, and
\Cref{sec:methods:prereg} says exactly what that does and does not
buy.

\subsection{Pre-registration and sealed spec}
\label{sec:methods:prereg}

\paragraph{The rank was not in the original seal, and this matters.}
The original E18 seal of 18 April 2026 specified the analysis unit,
the residualization, the bootstrap, the acceptance criterion and a
prohibition on post-hoc re-specification --- but it did \emph{not}
specify the rank $r$ at which \texttt{null\_ratio} is computed. In the
23 April development sweep over the 13 captured ranks
$\{1,2,3,4,5,8,13,16,21,32,34,55,64\}$, the gate changed from
\textbf{0 of 3 primaries passing at $r{=}32$ to 3 of 3 at $r{=}1$} --- a
complete inversion across an unpinned parameter. We therefore treat
that sweep as \textbf{exploratory}. A v3.1 amendment fixed $r{=}1$
before the replicate dataset existed, motivated both by the
commit-direction interpretation given in \Cref{sec:methods:pipeline}
and, secondarily, by $r{=}1$ yielding the largest-margin passing
intervals in the development data. Everything reported below evaluates
that \emph{amended} specification. It is not confirmation of an
originally rank-pinned E18 test, and we do not claim it as one.
The preliminary and powered runs share a generator, prompt family and
seed ($20260423$), and they overlap. We quantify that exactly, because
it bounds what the powered reading can be called.

The generator emits the four $2\times2$ cells in a fixed order
(ctrl/cl$=$2, contr/cl$=$2, ctrl/cl$=$5, contr/cl$=$5), drawing
\texttt{n\_per\_cell} puzzles for each from one sequential random
stream. The preliminary run drew $50$ per cell and the powered run
$150$, so the two runs begin from the same stream state and diverge as
soon as their per-cell draw counts differ. The consequence, verified
identically on all three primaries: \textbf{the first cell's $50$
puzzles are shared between the runs} --- byte-identical generated text
\emph{and} byte-identical token surprise --- while the remaining $150$
preliminary rows are fresh draws not present in the powered run. The
powered run therefore reuses $50$ of the preliminary run's $200$ rows,
$25\%$, and \textbf{is neither an independent replication nor a strict
superset.}

Two cautions for anyone re-analysing these artifacts. First,
\texttt{sample\_id} is \texttt{arange}$(N)$, and which cell a given
index falls in depends on \texttt{n\_per\_cell}; joining two runs on it
is meaningless. Second, the resulting cross-run agreement rates are a
pure artifact of that block arithmetic --- at matched index the runs
agree on the contradiction label $75\%$ of the time and on chain length
$50\%$, and both figures fall directly out of the $50$-versus-$150$
block boundaries rather than telling you anything about puzzle
identity. No content-bearing row key was banked, so the overlap above
had to be recovered by cell-aligned comparison of generated text and
surprise. \Cref{sec:disc:limitations} states what this does and does
not license.
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  ...
  \item \textbf{How the reported AUROCs are oriented.} Unless explicitly
  identified as directional or held-out, reported $\auroc$ values use the
  \emph{oriented} quantity
  $\max(\auroc, 1-\auroc)$, reported with the sign that achieved it.
  \textbf{That sign is chosen using all labels in the scored cell},
  and each bootstrap resample retains the sign of its cell's point
  estimate rather than being re-oriented. Two consequences a reader
  should carry: under a null of no association the oriented statistic
  has expectation near $0.519$ at $n{=}600$ rather than $0.5$, so
  oriented AUROCs are not directly comparable to unoriented ones; and
  $\Doriented$ compares \emph{discrimination strength} allowing either
  polarity, so a cell with a true \auroc\ of $0.26$ enters as $0.74$.
  For E18 the direction was pre-registered (higher null\_ratio $\to$
  contradiction) and all three primaries returned $+1$, so no folding
  was exercised there. For E17b the polarity \emph{is} fitted on
  evaluation labels; the head-to-head should be read as a comparison
  under evaluation-fitted polarity, and deployment would require a
  sign calibrated out-of-sample. \Cref{sec:results:validity} tests this
  concern empirically by fitting the sign on one half and scoring the
  other; it also reports directional residualizer intervals.
...
NO}; gen\_step$=$1 is the model's \emph{first generated token}.
\textbf{That is not always a content token.} Mistral, Phi-3.5-mini and
Gemma 3-4B emit \texttt{\textbackslash n} at gen\_step$=$1 in
$100\%$ of samples (\Cref{tab:wu_axis}), so for half the lineup the
sealed plane reads a formatting token that precedes the answer rather
than the answer's first token. We keep the plane as sealed and flag the
consequence in \Cref{sec:disc:mechanism}.

\paragraph{Capture.}
For each sample, we run the model with greedy decoding for
max\_new\_tokens$=$14, capturing at gen\_step$=$1 the final-block
residual-stream hidden state at two adjacent \emph{token positions}:
$h_{\text{prev}}$ at the last prompt position and $h_t$ at the first
generation position. $\dh = h_t - h_{\text{prev}}$ is therefore a
positional difference at a fixed depth, not a difference across the
last block. Concretely, $h_{\text{prev}}$ is the second-to-last row of
the final block's activations under causal attention. We also capture
the final RMSNorm $\gamma$ vector
...
to the corrected one. The sealed E18 \emph{verdict} is unaffected by
the correction. We had described residualization against
\texttt{d\_F\_lowrank32} as \emph{absorbing} the coordinate mismatch;
measured on the run-02 rows, which carry both geometries, that holds on
one model and not the others. Comparing legacy against post-norm at
matched rows, the \emph{unresidualized} metric moves by $0.084$
(Llama), $0.010$ (Mistral) and $0.134$ (Qwen 2.5), while the
\emph{residualized} metric moves by $0.001$, $0.017$ and $0.076$.
Absorption is near-total on Llama, \textbf{absent on Mistral --- where
residualizing makes the metric \emph{more} sensitive to the mismatch,
not less} --- and partial on Qwen 2.5. What survives is the weaker and
sufficient statement: residualization preserved the E18 \emph{verdict}
under both geometries. It is not a general invariance, and we no longer
claim one. The
sealed E17b verdict on Qwen 2.5 flipped from $-0.166$ (FAIL) to
$+0.1495$ (PASS) under the corrected geometry --- the $+0.32$ swing is
the $\Jn$ correction's effect on the head-to-head, which has no
residualization step to damp it.

% =====================================================================
\section{Results}
\label{sec:results}

The main comparisons below use $n=600$ per model (the powered run-09 +
2026-04-27/run-\{01,02\} sweep), 1000 sample-level paired bootstrap
resamples, seed $= 20260423$, $\Jn$-corrected post-norm geometry;
\Cref{sec:results:validity} specifies its diagnostic procedures separately. The
$n=200$ prelim (2026-04-26/run-02) is reported parenthetically where
directly comparable. The sole exception is \Cref{tab:e17b} and
\Cref{fig:jn}, which additionally carry the two $n=200$ readings that
bracket the $\Jn$ correction; every row there is labelled with its $n$.
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.7\linewidth]{pri-figures/fig1_sealed_e18.pdf}
...
non-overlap 95\% CI vs.\ $0.5$ (\Cref{tab:e18,fig:e18}). Sealed bar is
2-of-3; we clear 3-of-3.
Bootstrap CIs are $\sim 33\%$ tighter than the $n=200$ prelim
(run-02). The $\sqrt{3}$ scaling implied by $3\times$ the data would give
$42\%$, so as with E17b the observed tightening falls short of the
independent-sample ideal. We report the widths without attributing the
shortfall to the cross-run overlap of \Cref{sec:methods:prereg}: that
overlap bears on how much \emph{independent} evidence the two readings
jointly carry, not on the marginal width of an interval computed within
the larger run alone.
Verdict structure replicates the prelim direction-for-direction.
The pre-registered robustness check at \texttt{d\_F\_topk32} residualization
also passes 3-of-3 at sign $+1$ --- Llama $0.8778$ $[0.850, 0.902]$,
Mistral $0.8702$ $[0.844, 0.897]$, Qwen 2.5 $0.6357$ $[0.591, 0.680]$ ---
within bootstrap noise of the \texttt{d\_F\_lowrank32} primary in every
cell.
...
\begin{table}[htbp]
  ...
  \toprule
  Reading & $n$ & Fisher (sign) & Raw (sign) & $\Doriented$ & 95\% CI \\
  \midrule
  Buggy 2026-04-24 (run-05) & 200 & 0.7665 ($-1$) & 0.9323 ($+1$)
    & $-0.1658$ & $[-0.240, -0.098]$ \\
  Corrected 2026-04-26 (run-02) & 200 & 0.9008 ($+1$) & 0.7513 ($-1$)
    & $+0.1495$ & $[+0.100, +0.201]$ \\
  \midrule
  \textbf{Corrected, powered (run-09)} & \textbf{600} & \textbf{0.8967 ($+1$)}
    & 0.7396 ($-1$) & $\boldsymbol{+0.1571}$ &
  ...
  (post-norm $\dh$ on post-norm basis) geometry. The first two rows are
  matched at $n=200$ and isolate the $\Jn$ correction: buggy FAILS with
  Raw decisive, corrected PASSES with Fisher decisive, a $+0.32$ swing.
  The third row is the powered reading of the corrected geometry at
  $n=600$ and is the sealed verdict this paper reports.}
  \label{tab:e17b}
\end{table}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{pri-figures/fig2_sealed_e17b.pdf}
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\linewidth]{pri-figures/fig3_jn_correction.pdf}
  \caption{$\Jn$ geometry correction effect on the sealed E17b verdict
  (Qwen 2.5). The left pair is matched at $n=200$, and both readings
  record seed \texttt{20260423} with identical rank, layer, generation-step
  and bootstrap settings, so the comparison isolates the basis coordinate
  frame. Buggy reading from 2026-04-24 (pre-norm $\dh$ projected onto
  post-norm basis, run-05): $\Delta = -0.1658$, FAIL with Raw decisive.
  Corrected reading from 2026-04-26 (post-norm $\dh$ projected onto
  post-norm basis, $\Jn$-consistent, run-02): $\Delta = +0.1495$, PASS
  with Fisher decisive --- a $+0.32$ swing at fixed $n$. The hatched bar
  is the powered reading of the corrected geometry at $n=600$
  (run-09, $\Delta = +0.1571$), the sealed verdict this paper reports.
  The sealed E18 \emph{verdict} (\Cref{fig:e18}) is unaffected --- it
  passes 3-of-3 under both geometries, though residualization does not
  absorb the mismatch uniformly across models
  (\Cref{sec:methods:jn}). Its \emph{magnitudes} are not:
  Qwen 2.5's E18 AUROC moves from $0.7445$ to $0.6687$ across the same
  matched $n=200$ pair. The correction is not uniformly favourable, and
...
discriminates inverted (sign $-1$) at this analysis plane
(\Cref{fig:e17b}; the correction itself is plotted in
\Cref{fig:jn}). $\Delta
\auroc = +0.157$ clears the sealed $+0.02$ bar by $7.9\times$.

\paragraph{The sealed head-to-head is understated, not inflated, by the
orientation rule.} \Cref{sec:methods:prereg} notes that reported AUROCs
are oriented. In this cell that works \emph{against} the reported
result. Fisher is not folded here --- its unoriented \auroc\ is
$0.8967$ --- but Raw is: unoriented it reads $\mathbf{0.2604}$, i.e.\ on
Qwen 2.5 at the sealed rank the static-basis baseline is
\emph{anti}-predictive, and folding reports it at its magnitude,
$0.7396$. Comparing both scores in the registered direction instead of
by magnitude gives $\Delta \auroc = 0.8967 - 0.2604 = \mathbf{+0.636}$,
four times the published figure. \textbf{The gate passes under either
convention, and the convention we pre-registered is the conservative
one.} We report $+0.157$ because it is what the sealed spec defines,
not because it is the larger number.

\paragraph{A threshold metric makes the orientation hazard legible.}
Oriented \auroc\ maps an inverted score onto the same $[0.5, 1]$ range
as a working one, so a reader cannot see the difference. A
threshold-based metric evaluated in the \emph{registered} direction
can. Scoring at the median (predicting the top half as contradictions,
which matches the $50\%$ base rate of the $2\times2$ design and is not
fitted to labels), the sealed Qwen 2.5 cell gives Fisher $F_1 = 0.803$
and \textsc{Raw} $F_1 = \mathbf{0.240}$ --- against $0.50$ for a coin
flip. \textsc{Raw} is not a weak detector at this operating point; it
is a \emph{worse-than-chance} one, which its folded \auroc\ of $0.7396$
conceals. The same contrast appears at Gemma 3-4B $r=2$, where the
``Fisher decisive'' reading of \Cref{sec:results:motifs} scores $F_1 =
0.200$ in the registered direction. \emph{(With a balanced design and a
median threshold, precision, recall and $F_1$ coincide by
construction, so only $F_1$ is quoted. These are descriptive operating
points, not part of any sealed gate.)}

The $+0.32$ swing between the first two rows of \Cref{tab:e17b} is the
$\Jn$ correction's effect on the head-to-head
(\Cref{sec:methods:jn}); those two rows are matched at $n=200$, so the
swing is attributable to the coordinate frame and not to sample size.
We report all three rows for transparency. The powered reading
agrees with the $n=200$ corrected prelim ($+0.1495$ $[+0.100, +0.201]$)
within bootstrap noise --- though the two share $25\%$ of their rows
(\Cref{sec:methods:prereg}), so this is agreement on a partially
overlapping sample, not independent replication. The interval narrows by $36\%$
($0.1005 \to 0.0643$); the $\sqrt{3}$ scaling implied by a $3\times$
sample-size increase would give $42\%$, so the observed tightening
falls slightly short of the independent-sample ideal.
...
per stratum. The $6\times 13\times 2 = 156$-cell landscape
(\Cref{fig:landscape}) exposes three structurally distinct
architecture-dependence motifs.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=\linewidth]{pri-figures/fig4_rank_landscape.pdf}
  ...
  ranks (Motif 1). Gemma 4B appears to flip F $\to$ R sharply at
  $r=2 \to r=3$ (Motif 2), though that flip does not survive a fixed
  orientation.}
  \label{fig:landscape}
\end{figure}
...
every single rank in the sweep (\Cref{tab:phi}), every chain-length
stratum.
\texttt{null\_ratio\_raw\_post\_rank1} $= 0.9989$ --- nearly perfect
contradiction discrimination via the static $\Wu$ SVD basis alone.
\textbf{Phi is the canonical ``HARP-style \emph{contradiction}
discrimination works as advertised'' architecture, and the headline
counter-example to ``Fisher pullback uniformly wins.''} The qualifier
is load-bearing: Phi answers this benchmark correctly in $600$ of
$600$ cases (\Cref{sec:results:validity}), so nothing here shows that
its static basis detects \emph{errors}. The static basis carries the
rupture signal so cleanly that Fisher's per-sample reweighting cannot
add.

\subsubsection{Motif 2 --- An apparent within-model rank flip that
does not survive a fixed orientation (Gemma 3-4B)}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.85\linewidth]{pri-figures/fig8_gemma_rankflip.pdf}
  \caption{Motif 2 --- the apparent within-model rank flip
  (Gemma 3-4B), \textbf{plotted under the per-cell fitted orientation}.
  Three lines: pool ($n=600$, solid brown), cl$=$2 stratum ($n=300$,
  dashed orange), cl$=$5 stratum ($n=300$, dashed green). Both strata
  transition F $\to$ R together at $r=2 \to r=3$ (red dotted
  vertical). \textbf{The transition is real but unexplained:} neither
  metric changes sign across it, so it is not an orientation artifact,
  and no measurement here identifies a spectral or other mechanism.
  Note separately that Fisher is \emph{anti}-predictive at every rank
  shown, so under a single sign fixed in the registered direction Raw
  is decisive throughout. See the body text.}
  \label{fig:gemma}
\end{figure}

Under the orientation rule of \Cref{sec:methods:prereg} the flip from
Fisher to Raw is sharp at $r=2 \to r=3$: pool $\Doriented$ goes from
$+0.207$ (Fisher decisive at non-overlap CI) to $-0.211$ (Raw decisive
at non-overlap CI) within one rank step, and both chain-length strata
show the same transition.

\textbf{The transition is real; neither the spectral explanation nor an
orientation explanation is supported.} Two things must be separated.

First, the flip is \emph{not} produced by a sign change at the
boundary. Across $r=2 \to r=3$ Fisher's unoriented \auroc\ moves
$0.119 \to 0.373$ and Raw's $0.674 \to 0.837$: \textbf{both keep the
sign they already had}, so holding each metric's own sign fixed across
the sweep still yields $+0.208$ then $-0.211$. What changes is relative
discrimination \emph{strength} --- Fisher's falls with rank while
Raw's rises --- and that is a property of the data. An earlier version
of this paper attributed the flip to a spectral transition; a later
revision attributed it instead to Fisher crossing the folding
threshold. \textbf{Both are wrong}: Fisher does not cross $0.5$
anywhere in this region, and no measurement here identifies a
mechanism. We report the transition and claim no cause for it.

Second, and separately, \textbf{Fisher is \emph{anti}-predictive at
every rank across this region} --- unoriented $0.259$ at $r=1$, $0.119$
at $r=2$, $0.373$ at $r=3$. So ``Fisher decisive'' here means Fisher's
\emph{inverted} score discriminates more strongly, not that the
registered direction holds. Fixing both metrics to the registered
direction (higher null\_ratio $\to$ contradiction) makes Raw decisive
at all 13 ranks. Whether that matters depends on the question: for
discrimination strength under free polarity, the flip stands; for a
detector that must point the registered way without per-model
calibration, Gemma's Fisher score does not qualify at any rank.
...
\begin{table}[htbp]
  ...
  $\Dcross = -0.575$. It is not the grid maximum --- Qwen 2.5 at
  $r=16$ reaches $+0.742$ under the same rule --- and these magnitudes
  are not orientation-stable; see the body text.}
  \label{tab:mistral}
\end{table}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.95\linewidth]{pri-figures/fig9_mistral_simpsons.pdf}
  ...
  directions --- $\Dcross = -0.575$. Not the grid maximum (Qwen 2.5
  at $r=16$ reaches $+0.742$), and orientation-dependent; see the body
  text.}
  \label{fig:mistral}
\end{figure}

Mistral has \textbf{two Simpson's-paradox sites at non-overlap CI}
(\Cref{tab:mistral,fig:mistral})\textbf{:}
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  ...
  Raw-decisive at the stratum level}, so the pooled ``Raw'' verdict is
  a mixing artifact of the two chain-length subgroups.
  \textbf{This site is orientation-independent.} Because the orientation
  rule of \Cref{sec:methods:prereg} could in principle manufacture a
  pooled-versus-stratum reversal on its own, we re-derived this site
  with no folding and again under a single sign fixed on the pooled
  cell: \emph{all six unoriented AUROCs here exceed $0.5$}, so no cell
  is folded and all three rules return identical numbers. The reversal
  ...
  directions. $\Dcross = -0.575$ at $r=32$ is among the largest cross-stratum
  spreads in the model $\times$ rank $\times$ chain-length grid, but it
  is \textbf{not} the largest: Qwen 2.5 at $r=16$ reaches $+0.742$
  under the same rule, and under a fixed orientation the ranking
  changes again (Mistral $r=64$, $+1.006$). An earlier version of this
  paper claimed $-0.575$ was the grid maximum; that was incorrect and
  is withdrawn. Both of the larger cells are themselves heavily folded,
  which is the point: cross-stratum spread magnitudes are not
  orientation-stable and should not be ranked.
  \textbf{Unlike the $r=1$ site, this one is orientation-dependent and
  must be read with care.} Five of its six cells are folded: unoriented,
  Fisher reads $0.2606$ pooled, $0.4149$ at cl$=$2 and \textbf{$0.0279$
  at cl$=$5}. The ``Fisher decisive $+0.379$'' at cl$=$5 is therefore a
  near-perfect \emph{inverted} detector reported at its magnitude, not
  a detector pointing the registered way. The cl$=$5 Raw cell is also
  the one place where the locally fitted sign disagrees with the pooled
  sign, and that single disagreement moves $\Dcross$ from $-0.575$ to
  $-0.760$ under a fixed pooled orientation (and to $+0.760$ unoriented).
  The cross-stratum \emph{disagreement} survives every rule; its
  \emph{magnitude}, and the ``largest of 156'' ranking that depends on
  it, do not travel across orientation conventions.
\end{itemize}

Mistral is not unique in showing large cross-stratum spread: Qwen 2.5
exceeds $\pm 0.3$ at several ranks and reaches $+0.742$ at $r=16$,
which is larger than anything Mistral shows. What distinguishes
Mistral is that its spread is accompanied by \emph{non-overlapping}
stratum CIs in opposite directions at two separate ranks. An earlier
version of this paper stated that other architectures stayed within
$\pm 0.3$ throughout; that was incorrect. The mechanistic hypothesis
...
\begin{table}[htbp]
  ...
  Primary & surprise & PRI v1 cosine & PRI v2 topk32 & PRI v2 lowrank32
    & v3 rank-1 null ratio \\
  \midrule
  Llama 3.2 3B  & 0.6347 ($+$) & 0.6224 ($+$) & 0.7528 ($+$)
    & 0.7500 ($+$) & 0.8963 ($+$) \\
  Mistral 7B    & 0.5187 ($+$) & 0.5309 ($+$) & 0.6623 ($+$)
    & 0.6618 ($+$) & 0.7849 ($+$) \\
  Qwen 2.5 7B   & 0.8947 ($\mathbf{-}$) & 0.9155 ($\mathbf{-}$)
    & 0.7906 ($+$) & 0.7948 ($+$) & 0.8967 ($+$) \\
  \bottomrule
  \end{tabular}
  \caption{Baseline \emph{oriented} AUROCs on the sealed primaries at
  $n=600$, each with its fitted sign. The last column is
  \texttt{null\_ratio\_post\_rank1} \emph{unresidualized}.
  \textbf{It is not the sealed E18 metric}, which is the same column
  residualized against \texttt{d\_F\_lowrank32} and is reported in
  \Cref{tab:e18}; that is why Qwen 2.5 reads $0.8967$ here and $0.6468$
  there. The unresidualized form is the right comparator for these
  baselines, which are themselves unresidualized. The sealed v3 metric
  outperforms PRI v1 and v2 on Llama and Mistral by clear margins.
  \textbf{The Qwen 2.5 row needs its orientations read alongside it.}
  Surprise and PRI v1 cosine carry sign $-1$ there --- unoriented,
  $0.105$ and $0.085$ --- so their large oriented values report the
  magnitude of an \emph{inverted} association. v2 and v3 carry sign
  $+1$ on the same model. Two readings follow and we state both,
  because the column is oriented and an oriented column is a
  discrimination-strength measure. \emph{As discrimination strength},
  surprise and v1 genuinely are close to v3 on this model, and their
  inverted sign does not by itself disqualify them: a reliably inverted
  score is a usable detector once its polarity is calibrated.
  \emph{As detectors pointing the registered way} --- higher
  null\_ratio $\to$ contradiction --- only v2 and v3 qualify here. The
  distinction matters for deployment rather than for ranking, and it is
  the concrete case of the orientation caveat in
  \Cref{sec:methods:prereg}. We flag it because a reader scanning the
  row alone cannot see it, not because it settles which score is
  better.}
  \label{tab:baselines}
\end{table}
...
These descriptive checks use the banked gen\_step$=1$ rows and the
procedures in \texttt{scripts/validity\_panel.py}. They examine outcome
validity, score orientation, and residualizer uncertainty; they do not
re-specify E17b or E18, and the sealed verdicts remain unchanged.

\paragraph{Outcome ceiling.}
\textbf{The outcome ceiling is the most serious limitation of the
hallucination-detection motivation: five of six models answer at least
99.2\% correctly, leaving no or too few errors to detect.}
Error rates are 0.0\% for Mistral, Phi, and Gemma, 0.3\% for Llama,
0.8\% for Qwen 2.5, and 19.3\% for Qwen3-8B. The zero-error models
have no outcome variation at all; Llama and Qwen 2.5 have only 2 and
5 errors, respectively, among 600 samples each. For these five models,
the measured discrimination is a geometric response to contradictory
prompts that the model then handles correctly, not evidence of
hallucination detection. Only Qwen3 clears the panel's descriptive
5\% error-rate bar for examining error prediction --- and \textbf{that
examination does not survive inspection either}, for reasons given
immediately below, so the limitation covers all six models rather than
five. That bar is not a sealed acceptance criterion.

\paragraph{Folding incidence.}
Folding means that the score's directional $\auroc$ against the
contradiction label falls below $0.5$ and is reported as
$1-\auroc$. Across the 13 ranks, Fisher folds at 0/13 (Llama),
3/13 (Mistral), 6/13 (Qwen 2.5), 2/13 (Qwen3), 6/13 (Phi), and
11/13 (Gemma); Raw folds at 10/13, 7/13, 9/13, 13/13, 0/13, and
1/13 in the same model order. At sealed $r=1$, only Raw folds for
Llama, Qwen 2.5, and Qwen3; neither score folds for Mistral or Phi;
both fold for Gemma. \textbf{Folding is exercised in several sealed-plane
comparisons, so $\Doriented$ must retain its interpretation as a
comparison of discrimination strength allowing either polarity.}
The counts describe where inversion occurs; they do not establish
whether the fitted sign transfers to held-out rows.

\paragraph{Split-half sign stability.}
At $r=1$, the panel makes 200 random splits, fits the orientation
against contradiction labels on one half, and scores the other half
without re-orienting it. Agreement of the fitted sign with the
full-sample sign is 100\% in 10 of 12 model--metric cells; the
exceptions are Phi Fisher (99.0\%) and Gemma Raw (89.0\%). Mean
held-out $\auroc$ is 0.8971 for Qwen 2.5 Fisher, 0.8956 for Llama
Fisher, and 0.9988 for Phi Raw, free of in-sample orientation
advantage. Stability alone does not imply useful discrimination:
Phi Fisher reaches only 0.5557 and Gemma Raw only 0.5127 on held-out
rows, both near chance. \textbf{The orientation concern is
methodologically real but does not translate into sign instability
here, except at Gemma Raw, which is both the least stable and near
chance.} This is evidence for sign transfer within this dataset at
the sealed rank, not across ranks, prompt families, or deployments;
it does not overturn the rank-dependent orientation caveats in
\Cref{sec:results:motifs}.

\paragraph{Qwen3's apparent errors are a formatting artifact, and the
error-prediction check is therefore withdrawn.}
Qwen3-8B is the only model clearing the panel's $5\%$ error-rate bar,
so it was the only candidate for asking whether the geometry predicts
incorrect \emph{answers}. It does not survive inspection.
Qwen3 is reasoning-tuned, and under the sealed
\texttt{max\_new\_tokens}$=$14 budget it either leads with
\texttt{Answer:} or opens a chain-of-thought preamble that the budget
truncates before any answer appears. Partitioning its 600 generations
by first word is almost perfectly diagnostic: \texttt{Answer:} yields
484 scored-correct against 1 scored-incorrect, while
\texttt{Let's} (68), \texttt{Alright,} (37) and \texttt{Okay,} (10)
yield 115 scored-incorrect and none correct. \textbf{The outcome label
is fixed by the first generated token in 599 of 600 cases --- and
gen\_step$=$1, the position at which the geometry is measured, is that
token.} An error-prediction $\auroc$ computed on this label therefore
asks whether the hidden state at a position predicts which token
occupies it, which is close to tautological and is not evidence about
answer correctness. We withdraw the check rather than report it.

Two consequences follow. First, Qwen3's $19.3\%$ is a rate of
reasoning-preamble openings under a short generation budget, not a
hallucination rate, so \textbf{no model in this study supplies enough
valid outcome variation for an error-prediction evaluation}. Two models
do make errors --- 2 and 5 in 600 --- but at those counts an
error-prediction \auroc\ rests on too few positives to interpret. The
construct gap above therefore covers the whole lineup, not five-sixths
of it. Second, this reproduces a failure mode already documented
elsewhere in this project: for reasoning-tuned models the
gen\_step$=$1 token is frequently a preamble rather than a commitment,
which is a caveat on the analysis plane itself and not only on this
outcome label.

\paragraph{Residualizer uncertainty.}
The sealed analyzer fits OLS once and bootstraps the fixed residuals.
The panel compares this with refitting Fisher's $r=1$ regression on
\texttt{d\_F\_lowrank32} inside each of 1000 paired bootstrap
resamples, retaining the registered direction rather than folding
the residual $\auroc$. The 95\% interval widths change from fixed
to refitted as follows: Llama $0.0538\to0.0570$, Mistral
$0.0520\to0.0572$, Qwen 2.5 $0.0872\to0.0623$, Qwen3
$0.0933\to0.0992$, Phi $0.0902\to0.0715$, and Gemma
$0.0768\to0.0768$. \textbf{The effect is small and goes in both
directions, and no sealed verdict moves.} In particular, Qwen 2.5's
lower bound rises from 0.6035 to 0.6163, still clearing 0.60.
For these data, the check closes the objection that the fixed-residual
procedure determines the E18 outcome; it does not establish general
equivalence of the two bootstrap procedures.
...
on $N=100$ stratified puzzles per model (50 ctrl and 50 contr, split
evenly across
...
model's commit, so $\textsc{Raw}_\text{post,rank=1}$ scores high on
both --- $0.9254$ on Mistral and a near-saturating $0.9989$ on Phi ---
and Fisher's per-sample reweighting cannot add. Only Phi has a
near-ceiling contradiction $\auroc$ at the sealed rank (distinct from
the score-value saturation on Qwen3 in \Cref{sec:results:validity});
Mistral's Raw reaches $0.9991$ at
$r{=}2$, but that is off the sealed plane and is not what the sealed
comparison reads. Two architectures (Llama, Gemma 4B) have $V_\text{raw}[0]$
...
\texttt{Wikip}, \texttt{zna}) plus common short tokens. Neither is
...
The Qwen 2.5 baseline anomaly needs stating carefully, because its
direction is the interesting part. Surprise and PRI v1 cosine reach
oriented AUROCs near $0.90$ in \Cref{tab:baselines}, but both do so at
sign $-1$: unoriented, they sit at $0.105$ and $0.085$. On this model
and this benchmark, contradiction prompts are followed by a
\emph{less} surprising commit token than controls, not a more
surprising one. Whatever that reflects --- and we do not claim to know
--- it is not the intuitive reading that a contradiction perturbs the
model into hesitancy, and it is not a case of a simple scalar
recovering the sealed metric's performance. v2 and v3 carry sign $+1$
on the same rows. We flag this as an unexplained model-specific
inversion rather than fold it into the account above.
...
sealed E18 verdict is robust across the $\Jn$ correction, though its
magnitudes are not and residualization damps the mismatch on only some
models (\Cref{sec:methods:jn}). The sealed E17b verdict flipped
...
Fisher pullback, not to its definition.

\paragraph{What the buggy reading would have decided.} That flip was
not marginal, and its consequence was fixed in advance. The
pre-registration lists four \emph{project-falsifying} blockers; the
fourth reads: ``HARP's raw-$\Wu$ baseline equals or beats
Fisher-weighted \texttt{null\_bare} on Qwen (E17b): the
curved-geometry thesis collapses; plan must reposition toward HARP's
static formulation'' (\texttt{null\_bare} is the plan's internal name
for the Fisher-weighted null ratio). The clause allows no tolerance:
the E17b gate specification states the same condition as holding ``by
any margin.'' Under the buggy geometry the static raw basis did not merely
edge ahead: it read $0.9323$ against Fisher's $0.7665$
(\Cref{tab:e17b}). Had that reading stood, it would have triggered the
blocker, and the curved-geometry thesis on which this paper rests
would have been falsified on its own registered terms. Under the
corrected geometry the same sealed comparison clears the $+0.02$
acceptance bar by $7.9\times$ with Fisher decisive. What carries the
governance argument is what was \emph{not} done: the blocker was not
softened, rescoped, or amended, and the acceptance bar was not moved.
A coordinate frame in the implementation was repaired, and the sealed
comparison was re-run against an unchanged criterion.

We argue this is the correct
...
inconsistency in the prompt. This paper does not run a causal
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  \item \textbf{Outcome ceiling: the most serious limitation.
  \emph{No} model in this study exhibits a measurable error rate.}
  Five of six answer at least $99.2\%$ correctly --- Mistral, Phi and
  Gemma make no errors at all, and Llama and Qwen 2.5 make 2 and 5 in
  600. The sixth, Qwen3-8B, appears to err at $19.3\%$, but that number
  is a formatting artifact: its scored failures are chain-of-thought
  openings truncated by the generation budget, and the outcome label is
  fixed by the first generated token in 599 of 600 cases
  (\Cref{sec:results:validity}). \textbf{Every result in this paper is
  therefore a discrimination of contradictory \emph{prompts}, on
  prompts the models overwhelmingly answer correctly --- not a
  demonstration of hallucination detection.} The abstract's framing of
  the signal as a candidate substrate for such monitoring is a
  hypothesis this benchmark cannot test. Establishing it requires a
  task on which the models actually fail.
  ...
  is left to v4.
  \item \textbf{A later amendment on this line was falsified.} After
  the work reported here, a v3.2 amendment tested whether the
  \emph{centered} softmax-Fisher metric
  $\Wu^{\top}(\mathrm{diag}(p) - pp^{\top})\Wu$ --- the covariance form,
  rather than the uncentered $\Wu^{\top}\mathrm{diag}(p)\Wu$ pullback
  used throughout this paper --- would sharpen the null-ratio
  separation. It did not, and that amendment is recorded as falsified.
  The sealed E18 and E17b verdicts reported here are unaffected, since
  both are defined on the uncentered pullback; but the negative bounds
  how far the ``curved geometry'' reading should be pushed. The
  centered metric is not a strictly better object, and a reader should
  not infer that every Fisher-flavoured refinement of this construction
  improves on it.
...
specifically (the commit step). We previously described that step
as where the surprise-on-error gap and the geometric rupture gap show
their largest cross-architectural separation; that phrasing does not
survive \Cref{sec:results:validity}, since five of six models produce
essentially no errors and an on-error gap is undefined for them. What
the data supports is a separation on the \emph{contradiction} contrast,
measured at that step. The natural v4 question is therefore whether
...
a within-model transition in relative discrimination strength at
$r=2 \to r=3$ for which we identify no mechanism, spectral or
otherwise), and Mistral 7B has a chain-length $\times$ rank interaction
with two Simpson's-paradox sites, the larger showing
$\Dcross = -0.575$ at $r=32$ under the paper's orientation rule. We
...
\begin{description}[leftmargin=*, itemsep=4pt, topsep=2pt]
  ...
  standalone numpy diagnostic at $N=100$ across the three sealed
  primaries and Qwen3 8B;
  ...
  $-0.166$ (FAIL) to $+0.1495$ (PASS). The sealed E18 \emph{verdict} is
  unaffected --- 3-of-3 under both geometries --- though its magnitudes
  move (Qwen 2.5: $0.7445 \to 0.6687$ at matched $n=200$) and
  residualization damps the mismatch on only some models. The legacy pre-norm-$\dh$ code
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  \item \textbf{Code, two pins.} GitHub
  \texttt{flowstyleliving/PRI\_at\_commitment}. Commit
  \texttt{c5cedfa} (2026-04-27) is the submission state: it carries the
  frozen pre-registration, the sealed-gate analyzer
  (\texttt{scripts/analyze\_sealed\_gate.py}, post-norm geometry only
  after the 2026-04-26 cleanup), the run launcher and the pipeline
  (\texttt{pri\_v2\_mlx\_pipeline.py}). Commit \texttt{2ba666a}
  (2026-09-09, reachable from \texttt{main} via \texttt{b53d9a8}) is a
  post-submission revision of \texttt{scripts/make\_paper\_figures.py}
  and regenerates the figures in this manuscript; it changes no
  measurement. \textbf{Do not use the \texttt{v3-sealed} tag as the
  submission state} --- it dereferences to \texttt{635059c}, dated
  2026-05-13, two weeks after the run and the draft.
  \item \textbf{Pre-registration:}
  \texttt{PRI\_V3\_PRE\_REGISTRATION\_PLAN.md} at the repo root,
  frozen 2026-04-27. Byte-identical at \texttt{c5cedfa} and at
  current \texttt{main}.
  \item \textbf{What the public repository contains, and what it does
  not.} The repository tracks the \emph{scored} sealed-gate output,
  \texttt{experiments/v3-main-run/2026-04-26/run-09/sealed\_gate.json}
  (3 primaries $+$ Qwen3 at $n=150$/cell), and the rendered figures. It
  does \emph{not} track the per-row parquets those scores are computed
  from --- they are excluded by \texttt{.gitignore} --- and the two
  descriptive single-model runs
  (\texttt{2026-04-27/run-01}, Phi-3.5-mini; \texttt{2026-04-27/run-02},
  Gemma 3-4B) are not in the committed tree at either pin. A reader can
  therefore audit the archived gate numbers from a fresh checkout but
  \textbf{cannot re-derive them from it}; the banked inputs are
  distributed separately with checksums, and until that package is
  published this manuscript claims re-derivation only from data already
  in hand.
  \item \textbf{Hardware:} Apple Mac mini M4, 16~GB unified RAM.
  mlx-lm + mlx 4-bit quantized models from \texttt{mlx-community}.
  \item \textbf{Environment:} Python 3.9 + venv.
  \texttt{requirements.txt} at the submission commit \emph{names} ten
  packages but pins no versions; the exact resolved environment was not
  captured, and this is a real limitation of the deposit rather than a
  formality.
  \item \textbf{Re-score the banked analysis} (the verified path; this
  is what was checked, and it reproduced all 276 recorded fields):
  \begin{verbatim}
python scripts/analyze_sealed_gate.py \
    --run-dir experiments/v3-main-run/2026-04-26/run-09 \
    --out /path/to/scratch/rescored.json
  \end{verbatim}
  The \texttt{-{}-out} flag is \textbf{mandatory}: the analyzer
  otherwise defaults to writing \texttt{run-dir/sealed\_gate.json}, and
  would overwrite the sealed artifact under verification.
  \item \textbf{Regenerate from the models} (a different and
  \emph{unverified} path --- it re-runs generation, and MLX 4-bit
  generation determinism on this hardware has not been tested):
  \begin{verbatim}
git checkout c5cedfa
python -u scripts/run_v3_main.py --scope v3_1_main \
  ...
  \texttt{-{}-scope v3\_1\_gemma4b\_only}). Note that re-reading the
  sealed gate from a regenerated run is a fresh look at a single-look
  confirmatory endpoint, and is not how the verdicts reported here were
  obtained.
\end{itemize}
...
% point to arXiv / DOI / venue URLs. The three own-prior Kitti preprints
% are listed by title and date with NO URL, matching ace-draft.tex and
% rpv-draft.tex. Do not attach personal-drive share links: they expose an
% account, are not archival, and can change permissions or die. When those
% preprints get a DOI, cite the DOI here and in the other two drafts.
% =====================================================================
...
\newblock Semantic Entropy Probes: Robust and Cheap Hallucination
Detection in LLMs.
\newblock \emph{arXiv:2406.15927}, 2024.

\bibitem[Yeom et al.(2026)]{yeom2026commitment}
J.~Yeom, J.~Sok, H.~Kim, S.~Park, J.~Park, and T.~Kim.
\newblock Hallucination as Commitment Failure: Larger LLMs Misfire
