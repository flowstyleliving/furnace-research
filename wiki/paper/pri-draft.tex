...
of the commit step, with no fitted direction and no supervision target.
...
The development, preliminary and powered runs share a generator,
prompt family and seed ($20260423$), and we state plainly what the
banked artifacts can and cannot settle about their independence. The
result parquets key rows by \texttt{sample\_id}, which is
\texttt{arange}$(N)$ in every run and therefore carries no content: at
a given index the $n{=}200$ and $n{=}600$ runs agree on the
contradiction label only $75\%$ of the time and on chain length $50\%$
of the time, so the same index is a different puzzle. On the only
content-bearing evidence banked --- the 10 fully traced prompts per
run --- the two runs share \emph{none}. That is consistent with the
$n{=}600$ run being a fresh draw rather than an extension of the
$n{=}200$ run, but 10 prompts cannot establish it, and \textbf{no
content-bearing row key was recorded, so the artifacts cannot prove
non-overlap.} We therefore describe the powered run as a larger draw
from the same generator under the same seed, and do not claim it as an
independent replication. \Cref{sec:disc:limitations} states what that
...
\begin{table}[htbp]
  ...
  The third row is the powered replication of the corrected reading at
...
\begin{figure}[htbp]
  ...
  is the powered replication of the corrected reading at $n=600$
...
replicates the $n=200$ corrected prelim ($+0.1495$ $[+0.100, +0.201]$)
within bootstrap noise. The interval narrows by $36\%$
...
\begin{table}[htbp]
  ...
  $n=600$, each with its fitted sign. v3 sealed
  $=$ \texttt{null\_ratio\_post\_rank1}. The sealed v3 metric
