...
\begin{figure}[htbp]
  ...
  (Qwen 2.5). All three readings use the same sealed spec and the same
  post-norm basis; the left pair is matched at $n=200$ and differs only
  in the basis coordinate frame, so it isolates the correction. Buggy
  reading from 2026-04-24 (pre-norm $\dh$ projected onto post-norm
  basis, run-05): $\Delta = -0.166$, FAIL with Raw decisive. Corrected
  reading from 2026-04-26 (post-norm $\dh$ projected onto post-norm
  basis, $\Jn$-consistent, run-02): $\Delta = +0.150$, PASS with Fisher
  decisive --- a $+0.32$ swing at fixed $n$. The hatched bar is the
  powered replication of the corrected reading at $n=600$ (run-09,
  $\Delta = +0.157$), the sealed verdict this paper reports. Sealed E18
  (\Cref{fig:e18}) is unaffected by the correction because
  residualization absorbs the bias (\Cref{sec:methods:jn}).}
  \label{fig:jn}
\end{figure}
...
replicates the $n=200$ corrected prelim ($+0.150$ $[+0.100, +0.201]$)
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  ...
  commit \texttt{c5cedfa} (2026-04-27), which carries the frozen
  pre-registration, the sealed-gate analyzer, the run launcher and the
  run-09 artifacts. Pipeline:
  ...
  \texttt{experiments/v3-main-run/2026-04-26/run-09/} (3 primaries +
  Qwen3 at $n=150$/cell),
