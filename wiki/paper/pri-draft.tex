...
contradictions. \textbf{No model in this study exhibits a measurable
error rate: five of six answer at least 99.2\% correctly, and the
sixth's apparent 19.3\% is a truncated-chain-of-thought formatting
artifact. Every result here therefore discriminates contradictory
...
\begin{table}[htbp]
  ...
  \textbf{The Qwen 2.5 row must not be read as a near-tie.} Surprise
  and PRI v1 cosine carry sign $-1$ there: their unoriented AUROCs are
  $0.105$ and $0.085$, i.e.\ both are strong \emph{anti}-predictors of
  contradiction, and the large oriented values report the magnitude of
  an inverted signal rather than performance in the registered
  direction. v2 and v3 carry sign $+1$ on the same model. So on Qwen
  2.5 the sealed metric is not narrowly beaten by simpler baselines ---
  it is the only one of the three families pointing the registered way
  at high discrimination. This is the concrete case of the orientation
  caveat in \Cref{sec:methods:prereg}, and it is why deployment
  requires an out-of-sample sign.}
  \label{tab:baselines}
\end{table}
...
hallucination rate, so \textbf{no model in this study exhibits a
measurable error rate} and the construct gap above is total rather than
five-sixths. Second, this reproduces a failure mode already documented
