...
independent-sample ideal --- consistent with the runs not being
independent draws (\Cref{sec:methods:prereg}).
...
\textbf{This motif does not survive a fixed orientation, and we report
it as a negative.} Folding fires at $11$ of Gemma's $13$ ranks, and
Fisher is \emph{anti}-predictive across the flip region: unoriented,
its AUROC is $0.259$ at $r=1$ and $0.119$ at $r=2$, so the ``Fisher
decisive'' readings below $r=3$ are a near-inverted detector reported
at its magnitude. Fixing one sign per metric in the registered
direction --- higher null\_ratio $\to$ contradiction --- gives
$\Doriented$ of $-0.213$, $-0.555$ and $-0.464$ at $r=1,2,3$:
\textbf{Raw decisive at every one of the 13 ranks, with no flip
anywhere.} What the fitted rule renders as a rank flip is the rank at
which Gemma's Fisher score crosses the folding threshold, not a
transition in the SVD spectrum. The methodological point the flip was
recruited to make --- that rank is a commitment rather than a free
parameter --- is made independently and more cleanly by the $0/3$
versus $3/3$ E18 inversion in \Cref{sec:methods:prereg}.
...
an apparent within-model rank flip that we show is orientation-induced rather than a property of the SVD
