...
contradictions. \textbf{Five of the six models answer at least 99.2\%
correctly on this benchmark, so their results measure geometry under
contradictory prompting handled correctly, with no or too few errors
to support hallucination detection.} The head-to-head, the analysis plane (final
...
\paragraph{Actual error prediction on Qwen3.}
On the one model where this is testable under the panel's error-rate
bar, Fisher at $r=1$ gives an error-prediction $\auroc$ of 0.8906
pooled and 0.8570 within the contradiction arm. Surprise gives
0.9167 pooled but 0.7966 within that arm. \textbf{The within-arm
Fisher result holds prompt type fixed, so detecting the manipulation
alone cannot satisfy this check.} These error-prediction values use
evaluation-fitted folding separately for the pooled and within-arm
samples; they are descriptive associations with actual incorrect
answers, not held-out detector validation. Raw at $r=1$ is flagged
as saturated: within the contradiction arm, all values lie within
$4.2\times10^{-3}$ of $1.0$, with separation in the fifth decimal
of a score pinned at its ceiling. We do not interpret this numerical
separation as a perfect error detector. The Fisher result supports
error discrimination on Qwen3 in this benchmark only.
...
\begin{itemize}[leftmargin=*, itemsep=2pt, topsep=2pt]
  \item \textbf{Outcome ceiling: the most serious limitation.} Five
  of six models answer at least 99.2\% correctly on this benchmark.
  Mistral, Phi, and Gemma make no errors; Llama and Qwen 2.5 make too
  few to support the hallucination-detection claim that motivates
  the abstract. Their results measure the geometric response to
  contradictory prompts the models then handle correctly. Only Qwen3
  supports the panel's descriptive error-prediction check, including
  a within-contradiction comparison that holds prompt type fixed
  (\Cref{sec:results:validity}); this does not establish hallucination
  detection across the model lineup or beyond this benchmark.
