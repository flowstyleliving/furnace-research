...
\date{Original draft 2026-06-07 \quad This version: 2026-09-08}
...
\citep{farquhar2024semantic}. A reliable
...
monitoring. Single-pass operation alone no longer distinguishes such a
signal: semantic entropy probes \citep{kossen2024sep} recover a
resampling-based uncertainty score from one generation by fitting a
supervised linear probe on the residual stream, relocating the
sampling cost to calibration time. The distinction we pursue is
instead \emph{what is read}---an unsupervised geometric property of
the commitment step itself, rather than a fitted direction predicting
an uncertainty score. Independent work supports the choice of locus:
\citet{yeom2026commitment} align generation trajectories on the step at
which an answer concept is emitted and report that the correctness-predictive
information in the output distribution is localised there---detection
$\auroc$ is at chance two or more steps earlier, rises to $0.58$ at the
commitment step, and decays to chance within a few steps after---while
plain generated-token probability stays flat across the same window. The
effect is modest in size and sharp in position.
...
from $\Wu$-free attention features. Grounded in information geometry on the probability
...
\paragraph{An external account of what the spectrum may be reading.}
RPV is defined geometrically and we make no mechanism claim for it. It is
worth recording, however, that independent work describes a phenomenon of
the right shape. \citet{yeom2026commitment} group token completions into
answer \emph{concepts} and measure, at the commitment step, how the mass
already assigned to the correct concept is distributed across that
concept's surface forms. Comparing hallucinations against correct
generations matched on total concept mass, they find the distinguishing
factor is not whether the correct answer is represented but whether any
single form is concentrated enough to win the argmax: on
\texttt{Qwen3.5-9B-Instruct}, maximum single-form probability $0.26$
against $0.78$. That effect size is from the one model; what they report
across all $18$ they test is the \emph{sign}, which holds without
exception. Dispersion of the commit-step distribution is
exactly the kind of quantity a spectral reading of $\Fc$ is sensitive to,
and it offers an account of why RPV separates from plain confidence
(\cref{sec:results-conf}) that does not require confidence and commitment
to be the same variable---token-level confidence can be high while mass
is spread across forms of the right answer. Two limits keep this a
remark rather than a result. Their statistic requires ground-truth answer
aliases as input and is presented as an analytical probe rather than a
deployable detector, so it is not a comparator for anything reported
here; and whether the centered spectrum in fact tracks surface-form
dispersion is an empirical question we have not tested. We flag it as the
most concrete available hypothesis for the convergence in
\cref{sec:results-h1}, and register it as untested.
...
\bibitem[Kitti(2026)]{kitti2026pri}
M.~S.~R. Kitti.
\newblock Architecture-Dependent Fisher Pullback Geometry at the Commit
Moment: A Sealed Pre-Registered Test on Synthetic Logical
Contradictions.
\newblock Furnace Research preprint, 2026.
