...
### Status 2026-09-06 (later same day) — REVISED at seven metrics

The three-metric read above sampled 3 of the **seven** metrics the deployed panel
(`ATTENTION_PANEL_T0_WITH_V_NORMS`) actually uses. All seven now have depth data.

- **Two of the seven are dead at every depth.** `v_norm_bos` peaks at median **0.551** (0/17
  cells above the registered 0.65 bar); `v_norm_max` at 0.616 (1/17). **Neither is selected in
  any of 85 cross-fitted folds.** Their near-zero correlations with everything are **noise**.
- **The two-signal partition does not survive.** Among the five live metrics, correlations run
  0.33–0.90 with no clean split. `js` is closer to the sink family (0.632 / 0.725) than to
  `js_no_bos` (0.513). **`js_no_bos` is the least-coupled member of one broad family**, not a pole.
- **`js_kv_groups` is the modal fold winner (30/85)** with the panel's second-highest peak AUROC
  (0.929), and had never been analysed at depth.
- **Fusion-loses-to-best-single-column is restored at the deployed 21-column set** (−0.0145 A /
  −0.0457 B), reversing the grid-A wash reported at nine columns.
- **Method lesson for this candidate:** the instrument set a question is asked over is itself a
  choice that can manufacture the answer. Three metrics gave a clean two-pole story; seven gave
  one broad family plus two dead columns. **Enumerate the full deployed set before partitioning
  it** — the same class of error as the KV-tension comparator lesson.

**Open and now sharper.** Whether `v_norm_bos` / `v_norm_max` earn their place in the deployed
panel is a live question this read cannot answer — it measures per-layer separation at t=0, not
their contribution as fused cells under sealed-era signs. That is a cheap, well-posed follow-up.
...
> **Status at a glance (2026-08-17):** standalone verdict = **beats confidence / H1 NO-GO vs v3 / complements only in v3-collapse regime** (2026-06-07 comprehensive run, below). **But RPV did not end there:** it is a full family in the Commit-Confluence 29-cell panel, its `fisher_eff_rank` is one of the three fixed components of `fusion_rank_mean_geom` (with ACE_modal + `null_ratio_post_rank1`; `commit-confluence/stage_b/fusion_signs.json`), and RPV cells win 4/18 sealed deployable cells + both Llama-3.3-70B torch cells. The honest-negative is a *standalone-detector* claim; the confluence role is a *panel-member* claim. → [[claims]] §10 · [[results/confluence-seal-2026-06-11]] · [[results/bench-a2-signflip-2026-07-22]].

> **External prior art added 2026-09-06 — a testable mechanism, not a status move.** Status is unchanged (still beats-confidence / H1 NO-GO / collapse-regime-only). But [[lit/external]] now holds Yeom et al. 2026 (arXiv 2605.22007), which supplies the nearest external account of *what the spectrum may be reading*: at the commitment step, hallucination and correctness differ not in whether mass sits on the right answer concept but in whether it is **concentrated on one surface form or dispersed across aliases** (Qwen3.5-9B-Instruct, 0.26 vs 0.78; sign holds in 100% of 18 models). Dispersion of the commit-step distribution is the class of quantity a centered-Fisher spectral reading is sensitive to. **The concrete open question this hands #10:** does the centered spectrum in fact track surface-form dispersion? That is cheap to test on banked data and would discriminate "RPV is redundant with v3" from "both read one distributional event." ⚠️ Their statistic is an **oracle** (needs ground-truth aliases; explicitly "not a deployable detector"), so it is **not a comparator** and its AUROCs must not enter any comparison table. Tagged `[HYPOTHESIS — external prior art, untested here]` in [[claims]] §10; recorded in `rpv-draft.tex` §Discussion as an explicitly untested remark.
...
## 13. Depth-grid cross-family expansion — DC paper spine

**Status: [OPEN — WORKORDER FILED 2026-08-17].** Full plan: [[workorders/depth-grid-expansion-workorder-2026-08-17]] (grid, endpoints, engineering, costs). Paper scaffold: [[paper/dc-scaffold]].

**One-line**: extend the registered per-layer depth-curve instrument from 8 cells to a pre-registered 4-family × scale grid (Qwen anchor + Llama 3.1-8B/70B(/405B) + Mistral Small 3.2 / Medium 3.5 + Gemma 3 12B/27B) to confirm or kill the dip/cliff regularities out-of-family and resolve the Llama lineage question.

### Motivation

Grid A ([[results/depth-curve-2026-08-16]]) produced two candidate regularities (terminal-block dip 8/8; one-block CLIFF onset 7/8), one non-law (E1 placement UNDECIDED), and one methodological headline (the 3-rung panel's Llama blind spot). All of it rests on two families. The regularities are only paper-grade if they survive families the instrument has never seen; the blind-spot lesson is only fair to publish alongside that out-of-family test.

### Proposed mechanism

Same capture instrument (one mode grid-wide), upgraded statistics per the Codex round-1 audit: **cross-fitted E5** (peak selected on training folds, dip estimated held-out — the in-sample argmax contrast was positively biased), **directional permutation-calibrated E6** gatekept behind E5, frozen denominators (12 core cells; gate-aborts/no-peaks = failures; 405B outside all confirmatory denominators). Same two frozen datasets (sha-pinned), same gates (o_proj cos ≥ 0.999, YES/NO commit, zero drops). Engineering: registry + shared decoder descriptor for Mistral3/Gemma3 wrappers + multi-GPU ≥128B; Medium 3.5 is an FP8 checkpoint → one disclosed precision-heterogeneous cell. Prospective pinning of HF revisions/image digest/dependency versions/prompt-token hashes. Grid A = discovery; grid B = **prospective held-out-model confirmation on two fixed benchmarks**.

### Decision criteria for promotion

- E5 CONFIRM (cross-fitted, bars frozen at prereg) ⇒ the dip becomes the DC paper's registered regularity; FALSIFY ⇒ honest-negative reframe (grid-A regularity was family-local).
- E8 (descriptive) reports whether the Llama band appears in the same-size version comparison (3.1-70B vs banked 3.3-70B) and whether it is present at 8B — no causal vocabulary.
- Frozen `PRE_REGISTRATION_EXPANSION.md` before any launch (freeze-time Codex audit targeted GREEN); scorer runs once; misses reported as written.
- **[2026-08-17 — grid-A rescore DONE, freeze unblocked]:** under the exact cross-fitted estimators (built → Codex round-3 code audit, 8 MAJORs fixed → round-4 verification), grid A reads **E5 dip 8/8** (Δ_cf 0.111–0.416, pooled perm p ≈ 0.0005) and **E6 cliff 7/8** (sole miss structural: early peak ⇒ empty window). Both regularities survive debiasing; E6 stays confirmatory-gatekept. All 7 grid-B model revisions pinned (HF grants landed same day). → [[results/depth-rescore-2026-08-17]].
- **[2026-08-17 — GRID B RUN COMPLETE, registered verdict: E5 WEAKEN 8/12; E6 NOT TESTED.]** Frozen @ `2062e56` after Codex rounds 5–9 (RED→GREEN), gates-only smoke on one pinned toolchain, staged detached extraction 12/12 terminal, single-look scoring. The dip generalizes to every evaluable cell but one (p_grid 0.0005) yet misses the frozen CONFIRM bar; three failure modes (behavioral gate; **js_no_bos instrument-domain boundary on gemma-27b under extreme BOS sink**; one true miss). **P8/P9 hit: the Llama band replicates at 3.1-70B, absent at 8B.** DC paper now has both grids. Remaining scope: 405B stretch (MK go/no-go); MoE = next step. → [[results/depth-grid-2026-08-17]].

### Cross-references

- [[results/depth-curve-2026-08-16]] (grid A) · [[results/depth-marginals-2026-08-16]] (the superseded 3-rung read) · [[workorders/depth-grid-expansion-workorder-2026-08-17]] · [[paper/dc-scaffold]] · #5/ACE (the sealed metrics the kernel computes) · `commit-confluence/exploratory/depth-curve/`.
