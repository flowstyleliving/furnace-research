...
_Last updated: 2026-09-06 (THREE INSTRUMENTS, TWO SIGNALS, descriptive, $0 — candidate #16 step 1 finally done: the additive per-layer `v_norm` capture ran 2026-09-02 and all 17 cells joined. **`bos_mass` and `v_norm_lastq_weighted` are nearly the same depth curve — Pearson r median 0.896 across all 17 cells** (grid B 0.925), sharing a peak block in 8/17, while **`js_no_bos` stands apart from both** (0.329 / 0.384, peak shared 2/17) and every mutually-blind pair involves `js`. So the ACE attention panel reads **two** things, not three: inter-head disagreement, and a magnitude-and-sink family. Consequence: adding the third instrument to the depth-selectable set moves the primary contrast by a median of **exactly 0.0000** in both grids — not weakness (targeted `v_norm` alone beats the 2-instrument fixed panel by +0.1485 in grid A) but redundancy. All 2026-08-31 verdicts stand; one qualification, fusion-vs-best-single-column becomes a wash in grid A at nine columns. Method finding: the row-identity guard refused 5/17 cells on 1-2 ULP float noise before being calibrated to the data's own resolution — the working fix for the vacuous `sample_idx` check. NOT registered; [[three-instrument-2026-09-06]]). Prior: 2026-08-31 (DEPTH COVERAGE + INSTRUMENT REDUNDANCY, both descriptive, $0 — candidate #16 steps 2 and 3 run on banked artifacts. **Step 3, the discriminator:** a per-model depth-targeted single instrument beats the fixed aggregate 7/8 (grid A) and 7/9 (grid B), but against the harder opponent — the *best single fixed rung* — it **splits**: grid A 6/8, median **+0.1195**, 6/8 conditional CI exclude 0; grid B 6/9, median **+0.0110**, 4/9 exclude 0 in BOTH directions with Llama-3.1-8B/anli a significant **loss** (−0.075). Grid B is heterogeneous, not null. More damaging to the shared-latent reading and needing no appeal to depth: **fusing six fixed-rung columns loses to the best single fixed-rung column** (−0.0435 / −0.0383). Firmest positive = two-instrument targeted pair on held-out models (7/9, +0.0560, 7/9 exclude 0, 0/9 convention disagreements). **Step 2, fixed-depth redundancy:** the three instruments are **not** one variable — PC1 ≥0.90 in only 5/42 cells, ≥2 instruments clear a 0.60 residual bar in 21/42, and `bos_mass` is the most independent (R² 0.111). All 5 near-collapse cells are Llama-3.3-70B `[HYPOTHESIS]`. Two-round Codex audit: two conclusions **retracted**, neither changed a headline; the `sample_idx` row guard found substantively vacuous (data-contract gap). Selection-stability gate **PARKED**, not reported — post-hoc and threshold-dependent. NOT registered; sealed + registered verdicts untouched; [[depth-coverage-2026-08-31]], [[instrument-redundancy-2026-08-30]]). Prior: 2026-08-29 (INSTRUMENT CO-LOCATION, descriptive, $0 — the ACE attention cells are **not** depth-redundant: `js_no_bos` and `bos_mass` trace weakly-correlated depth curves (task medians ANLI 0.445 / HaluEval **0.127**) and **each is at chance where the other peaks in 6/17 cells**; peak-*location* test inconclusive (2/17), so the claim is about curve shape, not argmax. `bos_mass` — recorded per-block in both grids and never scored by either registered run — matches or beats the registered primary in **11/17**, and its Llama-3.3-70B/anli 0.915 exceeds both the js band 0.897 and the panel readout winner 0.816, enlarging the P3 blind spot. `v_norm_lastq_weighted` has no per-layer data. New candidate #16; hypothesis on the table that the fixed aggregate transfers via depth coverage rather than a shared latent. NOT registered; sealed + registered verdicts untouched; [[instrument-colocation-2026-08-29]]). Prior: 2026-08-17 later (GRID-B REGISTERED VERDICT — **E5 terminal-dip WEAKEN 8/12** on the held-out-model grid (6 new models × 2 tasks; every evaluable cell but one passes, p_grid 0.0005, pooled dip CI [0.123, 0.201]; misses the frozen ≥10/12 + family bars — reported as written); E6 NOT TESTED (gatekept); **Llama mid-stack band replicates at 3.1-70B, absent at 8B (P8+P9 hit)**; new instrument-domain finding: sealed `js_no_bos` undefined under gemma-27b's extreme BOS sink; Medium-3.5 FP8-origin-dequant passes both cells; freeze `2062e56` → results `cdc55a9`, nine-round Codex dialogue, [[depth-grid-2026-08-17]]). Prior: 2026-08-17 (GRID-A RESCORE under cross-fitted endpoints — **dip and cliff survive debiasing**: E5 8/8 (Δ_cf 0.111–0.416, pooled perm p ≈ 0.0005), E6 7/8 (sole miss structural: early peak ⇒ empty window), four-round Codex dialogue RED→YELLOW→verified, grid-B bars now rest on honest discovery rates, [[depth-rescore-2026-08-17]]; depth-grid expansion workorder + DC paper line opened, all 7 grid-B model revisions pinned, [[../workorders/depth-grid-expansion-workorder-2026-08-17|workorder]]). Prior: 2026-08-16 (REGISTERED DEPTH-CURVE RUN, 8/8 cells clean — E1 absolute-vs-relative UNDECIDED, no transferable depth law; the three-rung "N−2 peak" dissolves at full resolution (true Qwen peaks N−6…N−17); rise is a one-block CLIFF in 7/8; **P3 registered miss = headline: Llama-70B negative control carries a broad mid-stack attention band (peak 0.897 > its panel readout 0.816) exactly where the three-rung panel never sampled — "Llama→readout" is panel-relative**; P5 8/8: universal terminal-block dip; Codex-audited pre-launch, sealed untouched, [[depth-curve-2026-08-16]]. Earlier same day: three-rung marginals + small-model sweep, superseded-in-part, [[depth-marginals-2026-08-16]]). Prior: 2026-07-27 (FIRST CLEAN JUDGED VALIDATION — semantic parse rate 1.0000, gate 0.95 cleared, $4.47 of a $22 cap; [R4] naming diagnostic built + a retracted degeneracy claim; E3-HaluEval descriptive label-cost sweep — knee at 150 labels, 10/10 flat to 500; cc-draft ↔ cc-extend merged into one comprehensive paper; EG cell panels standalone bit-identical, 174/174). Detail in [log](../log.md) tail._

## All seven ACE attention metrics at depth — two of them are dead (2026-09-06, extension)

Same session, run after the three-metric read below was already written. Full detail: [three-instrument-2026-09-06](three-instrument-2026-09-06.md) §Extension.

- **Two of the seven deployed metrics carry no signal at any depth.** `v_norm_bos` peaks at median **0.551** and clears the registered 0.65 bar in **0/17** cells; `v_norm_max` reaches 0.616 and clears it in **1/17**. Neither is selected in **any** of 85 cross-fitted folds.
- **Their apparent independence was noise.** Near-zero correlations with everything else reflect flat curves, not a distinct signal.
- **So the clean two-way partition does not survive.** Among the five signal-bearing metrics, correlations run 0.33–0.90 with no clean split: tightest pair `bos_mass`~`v_norm_lastq` **0.896**, second `js`~`js_kv_groups` **0.839**, and `js` sits closer to the sink family (0.632 / 0.725) than to `js_no_bos` (0.513). **`js_no_bos` is the least-coupled member of one broad family**, not one pole of two.
- **`js_kv_groups` is the modal fold winner (30/85) and had never been analysed at depth**, with the panel's second-highest peak AUROC (0.929).
- **Widening the selectable set changes nothing** — `target1_7 − target1_3` median exactly +0.0000 in both grids.
- **The fusion-loses-to-best-single-column result is restored at the full column set**: `fixed21 − rung_best1_7` = −0.0145 (A) / −0.0457 (B, 5/9 exclude 0). The grid-A wash at nine columns does not survive at twenty-one.
- **Depth-coverage verdicts untouched.**

## Three instruments, two signals — superseded in part by the section above (2026-09-06)

Descriptive, cross-fitted, **$0 of compute and not a registered endpoint**; the first read of candidate #16 that is not one instrument short. Full detail: [three-instrument-2026-09-06](three-instrument-2026-09-06.md).

- **The panel is two signals, not three.** `bos_mass` and `v_norm_lastq_weighted` trace nearly the same depth curve — Pearson r **median 0.896 across all 17 cells** (grid A 0.784 / grid B **0.925**) — and share an in-sample peak block in **8/17**. `js_no_bos` stands apart from both (median r **0.329** vs bos, **0.384** vs v_norm; shares a peak with bos in only 2/17). Every mutually-blind pair at the 0.65 bar involves `js`.
- **Adding the third instrument changes nothing where it counts.** The primary contrast `target1_3i − target1_2i` has a median of **exactly +0.0000** in both grids; the depth-targeted winner is unchanged in 7/8 and 7/9 cells, and the best-fixed-rung winner in 7/8 and **9/9**.
- **That is redundancy, not weakness.** A depth-targeted `v_norm` on its own beats the two-instrument fixed panel by median **+0.1485** in grid A (7/8 intervals exclude zero) and +0.0235 in grid B. It is a good instrument pointed at the same thing as `bos_mass`.
- **Every 2026-08-31 verdict stands**, including the grid-A/grid-B split, grid-B heterogeneity, and the Llama-3.1-8B/anli loss. `target3` vs best fixed rung is 7/8 and 7/9 with 7/9 grid-B intervals excluding zero — the same shape as the two-instrument result.
- **One qualification.** With nine fixed columns instead of six, fusion-versus-best-single-column becomes a **wash in grid A** (+0.0003, 4/8) while staying negative in grid B (−0.0280, 2/9). Adding a redundant column helps a rank-mean fusion more than a single-column arm. The held-out-model direction is unchanged.
- **Method finding — the row-identity guard had to be calibrated, not asserted.** Exact equality on all four mirrored columns refused **5/17** cells, all `anli_r1`, on disagreements of 1–2 rows at |Δ| ≤ **2.220e-16** (1–2 ULP of float64). `commit_p` is the only mirrored column with row-level resolution (200 distinct values vs 2/2/3), so it could not be dropped; its smallest genuine gap is **5.936e-08**. Tolerance set at **1e-12** — four orders above the artifact, five below the smallest real gap. All 17 then joined. This is the working replacement for the `sample_idx` guard found vacuous on 2026-08-31.
- **Join cross-checked.** The recomputed `js~bos` correlations reproduce the registered co-location medians exactly (0.517 grid A / 0.218 grid B) — the third instrument was added without disturbing the first two.
- **No mechanism claim.** 0.896 describes two depth curves on 17 cells. It does not establish that the two instruments compute the same quantity, though both being magnitude-flavoured reads of the attention write is consistent with it.

## Depth coverage — does aiming one instrument beat pinning several? (2026-08-31)

Descriptive, cross-fitted, post-hoc on banked artifacts, **$0 of compute and not a registered endpoint**. Full detail: [depth-coverage-2026-08-31](depth-coverage-2026-08-31.md).

- **The discriminator for candidate #16.** If the fixed ACE aggregate transfers by *depth coverage* rather than a shared latent, aiming one instrument at a model's own peak block should match or beat it. 17/20 usable cells, stratified 5-fold cross-fit, instrument + block + orientation fit on training folds only.
- **Against the fixed aggregate the hypothesis holds:** depth-targeting wins **7/8** (grid A) and **7/9** (grid B).
- **Against the best single fixed rung it splits — and the split is the finding.** Grid A **6/8**, median **+0.1195**, 6/8 conditional intervals exclude zero. Grid B **6/9**, median **+0.0110**, only **4/9** exclude zero and they point **both ways** — Llama-3.1-8B/anli is a significant loss at **−0.075** [−0.124, −0.026]. On models the instrument was developed against, the aggregate was buying depth coverage; on held-out models that does not survive as a general rule.
- **The aggregate is not even the best fixed thing.** Fusing six fixed-rung columns *loses* to the best single fixed-rung column — median **−0.0435** (A) / **−0.0383** (B), first-arm wins 3/8 and 3/9. This undercuts "the aggregate buys complementarity" **without appealing to depth at all**, and it was identical across all three scorings.
- **Firmest positive in the run:** the two-instrument depth-targeted pair vs the best fixed rung on held-out models — grid B **7/9** wins, median **+0.0560**, **7/9** intervals exclude zero, **0/9** convention disagreements (the only contrast in either grid with none).
- **One zero is definitional.** Qwen2.5-7B/halueval peaks at block 26, and with 28 layers the `N−2` rung *is* block 26 — both arms are the same column. That exact 0.0000 is what coverage looks like when it happens to work.
- **Two-round Codex audit, every finding re-verified by execution; two conclusions retracted, neither changed a headline.** (1) The `target1_gated` sensitivity arm was mathematically degenerate, so its "gate changes selection in 0/85 folds" measured nothing — gate-inertness now rests on the independent `training_peak_qualifies_0.65` = 5/5 in all 17 cells, and the registered shuffled-envelope limb is a **declared gap, not a passed test**. (2) Training-CDF calibration is a step map that tied distinct opposite-label rows (752 created, 205/255 fold AUROCs moved, max 0.010); cell-level effect on the primary contrast **max 0.0035, 0/17 sign flips**.
- **Verified vacuous:** the `sample_idx` row-identity guard equals `arange(200)` in all 17 files and cannot detect a permutation; no model-independent per-row key is banked. Nothing reported depends on it (every contrast is within-cell). Logged as a data-contract gap.
- **Not claimed:** the conditional interval charges nothing for selection, so procedure-level generalisation stays unsettled — median training top-vs-runner-up margin is **0.0094**, exactly the close-race regime the audit warned about. The **selection-stability** observation is **parked, not reported** (post-hoc, floor breaks at a 3/5 threshold, grid-confounded, endogenous).

## Instrument redundancy — are the three ACE attention cells one variable? (2026-08-30)

Descriptive, post-hoc on banked matrices, **$0 of compute and not a registered endpoint**; **signs fit in-sample**, so levels are optimistic and only relative structure is read. Full detail: [instrument-redundancy-2026-08-30](instrument-redundancy-2026-08-30.md).

- **The fixed-depth half of the co-location question**, and the only limb that can use all three instruments — the banked `profiles_ext` matrices record `v_norm_lastq_weighted` at each panel depth even though its *per-layer* data is missing. 42 non-variant (model, task, depth) cells, n=200.
- **The three instruments are not redundant in general; redundancy is cell-specific.** PC1 carries median **0.594** of the covariance and reaches ≥0.90 in only **5/42** cells. Pairwise |Spearman| median 0.379, range [0.001, 0.993].
- **21/42 cells have two or more instruments clearing a 0.60 residual-after-the-others bar** (three in 4 cells, two in 17, one in 12, none in 9).
- **No instrument is dominated and none is universally unique** — median residual AUROC 0.594 `js_no_bos` / 0.592 `bos_mass` / 0.573 `v_norm_lastq_weighted`, each clearing the bar in 19–20 of 42.
- **`bos_mass` is the most independent of the three** (R² from the other two: median **0.111** vs 0.447 js and 0.518 v_norm). The panel's least-examined instrument is its least redundant — same direction as the 2026-08-29 depth read.
- **`[HYPOTHESIS]` Near-collapse looks family-level.** All five high-PC1 cells are **Llama-3.3-70B** (every Llama cell 0.88–0.94; no Qwen cell above 0.82). One model, two tasks — a pointer, not a result — but it rhymes with the registered locus dissociation.
- **Redundancy is depth-dependent within a model.** On Qwen2.5-32B (widest coverage) PC1 falls **0.731 → 0.448** from `N−2` to `final` while cells with ≥2 independent instruments rise **4/8 → 7/8**.
- **Verdict.** The standing "one shared change in attention allocation" reading is **not supported as a general claim** — it holds in 5/42 cells, all one family — and is **not refuted** either, since 9 cells have no instrument clearing the bar. Coverage is unbalanced (Qwen2.5-32B 24 cells, the other three models 6 each).

## Instrument co-location — the ACE attention cells are not depth-redundant (2026-08-29)

Descriptive, post-hoc on banked artifacts, **$0 of compute and not a registered endpoint**. Full detail: [instrument-colocation-2026-08-29](instrument-colocation-2026-08-29.md).

- **The test was sitting in the data.** Both depth grids record `final_bos_mass` per block alongside the js family; neither registered run scored it. So the second ACE attention instrument has had full per-layer coverage all along. 17/20 cells usable (gemma-3-27b ×2 lost to the `js_no_bos` domain boundary, Mistral-Small/anli to the behavioral gate).
- **Curve shape diverges; peak location does not resolve.** Gap CI excludes zero in only **2/17** — argmax bootstrap bands are too wide at n=200 to carry a locus claim, so *never* say "different loci." What does separate is shape: Pearson r median 0.517 (grid A) / 0.218 (grid B), splitting by task into ANLI R1 **0.445** vs HaluEval-QA **0.127**.
- **Mutual blindness in 6/17 cells, one-way in 5 more.** At the registered ≥0.65 bar, both instruments fall below it at the other's peak. Load-bearing cell **Qwen2.5-72B/halueval**: js 0.896 @ block 63 (bootstrap pinned [63, 63]) reads **0.536** at block 70; bos 0.853 @ 70 reads **0.524** at 63; r = 0.116. Second exemplar Llama-3.1-8B/halueval at r = **0.016**.
- **`bos_mass` is not a secondary instrument** — it matches or beats the registered primary in **11/17** cells, including every Llama cell in both grids.
- **The P3 blind spot is bigger than reported.** Llama-3.3-70B/anli `bos_mass` peaks **0.915**, above the js band's 0.897 and well above the panel readout winner 0.816.
- **One instrument short.** `v_norm_lastq_weighted` was never captured per-layer; capturing it is the gating build step for candidate #16.
- **Open hypothesis, not a finding:** the fixed ACE aggregate may transfer through *depth coverage* rather than a shared latent — discriminated by a per-model depth-targeted single cell vs the aggregate.
