...
_Last updated: 2026-07-22 (BENCH strict Phase-4 confirmatory verdict — Family-A split + intrinsic sign-flip). Detail in [log](../log.md) tail._
...
- **[OPEN — observation]** the three-model flip cluster (Mistral-7B, Mistral-Nemo, Qwen2.5-7B) is exactly the v4 sealed **E_A2 partial-transfer** trio. Untested coincidence vs sub-family; cheap to check, do not state as a finding. A **designed-retrospective** Fisher-exact screen is now specced (`stage_b/SIGNFLIP_COINCIDENCE_DESIGN.md`, proposed — not run).
